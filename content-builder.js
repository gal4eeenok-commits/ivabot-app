...
/* v50: Frontend keyword highlighting — full phrases only */
const hlHtml=(rawHtml,kws)=>{if(!rawHtml||!kws?.length)return rawHtml;
/* Build keyword list sorted by length DESC (full phrases first) */
const kwList=kws.filter(k=>k.keyword).sort((a,b)=>(b.keyword?.length||0)-(a.keyword?.length||0));
if(!kwList.length)return rawHtml;
const hlOne=(src,kw,freq)=>{const esc=kw.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const re=new RegExp(`\\b(${esc})\\b`,'gi');const parts=src.split(/(<[^>]*>)/);return parts.map(part=>{if(part.startsWith('<'))return part;return part.replace(re,(m)=>{return`<mark data-freq="${freq}">${m}</mark>`;});}).join('');};
let out=rawHtml;
/* Highlight full keyword phrases only — no individual word highlighting */
kwList.forEach(k=>{out=hlOne(out,k.keyword,k.freq||"MV");});
return out;};
