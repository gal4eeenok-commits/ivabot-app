...
const buildPartialRegexes=(kw)=>{const words=kw.split(/\s+/).filter(w=>w.length>0);if(words.length<3)return[];const escaped=words.map(w=>w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'));const pairs=[];for(let i=0;i<escaped.length-1;i++){pairs.push({re:new RegExp(`\\b(${escaped[i]}(?:\\s+${STOP_WORDS}){0,3}\\s+${escaped[i+1]})\\b`,'gi')});}return pairs;};
...
/* partial match: consecutive 2-word pairs from 3+ word keywords, only where no full match exists */
