...
function CoreTool({ onHome }) {
  ...
  const runAudit = async (url) => {
    ...
    try {
      ...
      try { window.__ivaAuditReady = Date.now(); window.dispatchEvent(new Event("iva-audit-ready")); console.log("[IvaBot] audit ready signal sent"); } catch(e) { console.warn("[IvaBot] audit ready signal failed", e); }
