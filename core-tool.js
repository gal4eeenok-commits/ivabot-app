/* IvaBot CORE TOOL (core-tool.js) v267 - v267 - the charge call names the page it is for, so the wallet works the price out from the keywords stored for that page instead of trusting the number this file sends. The number is still sent and is still used when the wallet cannot price the run itself. v266 - a run that stops after the credit was taken now gives it back. charge_credit returns the id of its ledger row, the tool keeps it, and the error branch asks the credit-refund function to return that exact charge. The refund reads the account from the token, accepts one charge once and refuses a charge that belongs to someone else or is older than an hour. v265 - the sandbox reaches this tool. The flag set on the dashboard was known only to the dashboard, so a run started with iva_sandbox=1 still took a credit, still paid DataForSEO and still wrote a run row, a snapshot and a report into the live tables. The tool now reads the same key from localStorage, marks every DataForSEO call as sandbox, takes no credit and writes nothing, and it says so in the console. v264 - a page with no links records zero rather than nothing. The inner-page count already worked out zero links and zero referring domains from the page's own lists, and then wrote null into the snapshot, so the card showed a dash and read as not measured next to keywords and traffic that showed a plain zero. A missing value on an inner page is now stored as zero, which is what was counted; the home page keeps null, where an absent domain summary really does mean the number is unknown. v263 - the page itself is asked which market it serves. air-gpt v12 answers with a country code, a language and a confidence, read only from what the page shows, and returns ww when the page belongs to no single country; that answer is passed to detectLocale, which has taken it since it was written and still ranks a country-code domain and hreflang above it and ignores a low confidence. A site on a .com with no hreflang, which is most of them, therefore stops defaulting to the United States when its own text says otherwise. Nothing overrides a market already stored: the lookup returns it before any of this runs. v262 - the market belongs to the page. The lookup asks for the address of the page first and falls back to the site, so a page given its own market in the dashboard is checked in that market while its neighbours keep theirs. A page with no market anywhere keeps the one worked out from the page itself, written back under its address, and a market already stored, chosen by hand or detected on an earlier run, is never overwritten later, so positions stay comparable between runs. Aruba, Curacao and Caribbean Netherlands join the country table with the codes DataForSEO returns for them, 2533, 2531 and 2535. v261 - an inner page showed the backlinks of the whole domain. The summary endpoint answers for the domain even when the target is a page, while the itemized list and the referring domains answer for the page, which is why one run reported 12,094 links and 58 domains next to an empty list of both. An inner page now counts its own links from its own lists, so a landing page with no links of its own reads zero instead of borrowing the domain total. The home page is unchanged. v260 - the audit used to go ahead whenever the charge call failed for any reason other than an explicit refusal, so a network error or a timeout produced a free run that still cost money at DataForSEO. The run now starts only on a confirmed ok from the wallet, and says which of the two happened: not enough credits, or the charge could not be taken. v259 - a saved report opened from the dashboard now says which day it belongs to. The page carried no date at all, so an old report and a fresh one looked the same and the only way to tell them apart was the hint on the link that opened it. The date comes from the report row itself; if it cannot be read the old wording is used unchanged. v258 - the keywords the page is built for now take their position from a live SERP check through the existing keyword_positions mode, instead of being looked up in the DataForSEO Labs ranked list, where a keyword missing from the set showed a dash even when the page ranked. The fuzzy fallback in buildReportData is gone: it matched on a substring and then on word overlap above 0.5, so one keyword could carry another keyword's position and it read as fact. v257 - the audit-ready announcement now also fires on the two paths that put an existing report on screen, a page reload that restores the saved report and a stored report opened by ?report=, so the Dashboard link breathes whenever a finished audit is visible rather than only after a fresh run. The three call sites share one helper, ivaAuditReadySignal. v256 - when the finished report goes on screen the tool now announces it, by setting window.__ivaAuditReady and firing an iva-audit-ready event, which is what makes the Dashboard link in the shell nav start breathing. The announcement sits at the moment the report renders, before the snapshot and report writes, so a database failure cannot suppress it. v255 — each saved checklist item now also carries a short what-to-do line, taken from the eval suggestion or reason where the audit has one and falling back to a plain sentence, so the dashboard table can match the AI Readiness one column for column. v254 — the Core snapshot now also saves the audit checklist in p_coverage (bad: title + priority), built from the plain status fields so it cannot throw, and logged to the console. The dashboard reads it to list what to fix on the page. v253 — referring domains are now fetched in parallel with the itemized backlinks list and written into the same snapshot field, marked kind:"domain", so the dashboard can show a links tab and a domains tab. The on-screen report and the PDF are unchanged. v252 — PDF section order now matches the on-screen report exactly: SEO Score, How Your Page Ranks, What Your Page Is Built For, PR & Backlink Opportunities, Top Competitors, Page Context Summary, What's Working, Needs Improvement, Final Recommendations (previously Page Context sat near the top and Backlinks/Top Competitors sat after Needs Improvement). Pure reordering of content.push blocks; no content or logic changes. v251 — PDF now surfaces the page-level counts that the on-screen report already shows: Backlinks, Referring Domains and Ranked Keywords (the PDF block was gated behind if(false) until the real DataForSEO endpoints went live post 2026-07-01; that gate is removed so the PDF matches the screen). v250 — align PR & outreach opportunities header flush-left under Backlinks/Referring domains via new Fold headerPad prop (PR fold passes headerPad="14px 0"). v211 — AIR-style pass: rankings shown before 'built for'; data sections use AI-Readiness card style (title + Dashboard link with chart icon, metric rows value+period); backlinks page-level with collapsible PR opportunities; Export PDF/CSV removed (downloads live in dashboard). v210 — standalone tool registered as window.CoreTool, mirrors window.AIReadinessTool; whitelist-gated; embedded by the /app hub, renders only the Core tool body (no nav/select). Base v202 — CLOSED PREVIEW, whitelist-gated copy of seo-tools.js. Core report rebuilt to mirror AI Readiness order: collapsible Page Context Summary, What your page is built for, Positions (+ dashboard link), Backlink opportunities with counts (+ dashboard link), Top Competitors, then What is working / Needs improvement. Tracking over time lives in the dashboard. Live seo-tools.js untouched. Gate: window.__CORE_OPEN===true or user_id in CORE_WHITELIST. */
(function() {
const { useState, useRef, useEffect, useCallback } = React;
console.log("[IvaBot] core-tool.js v267 loaded (standalone window.CoreTool)");

/* v257: one place that announces a finished report to the shell nav, so the Dashboard link
   breathes in all three cases where a report reaches the screen: a fresh run, a page reload that
   restores the saved report, and a stored report opened by ?report=. Before this only the fresh
   run announced itself, so a reload left the link still. */
function ivaAuditReadySignal(){
  try { window.__ivaAuditReady = Date.now(); window.dispatchEvent(new Event("iva-audit-ready")); console.log("[IvaBot] audit ready signal sent"); }
  catch(e) { console.warn("[IvaBot] audit ready signal failed", e); }
}
...
const Fold = ({ title, children, open: d = false, borderColor, headerBg, titleColor, count, headerExtra, titleSize, headerPad }) => {
  ...
  return (<div style={{ borderRadius: 12, border: `1px solid ${borderColor || C.border}`, overflow: "hidden", background: C.surface }}>
    <button onClick={() => setO(!o)} style={{ width: "100%", padding: headerPad || "14px 16px", background: headerBg || "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", fontFamily: "'DM Sans',sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: titleSize || 14, fontWeight: 700, color: titleColor || C.dark }}>{title}</span>{count != null && <span style={{ fontSize: 11, fontWeight: 600, color: titleColor ? "rgba(255,255,255,0.7)" : C.muted, background: titleColor ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.5)", padding: "2px 8px", borderRadius: 10 }}>{count}</span>}</div>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>{headerExtra && <span onClick={e => e.stopPropagation()} style={{ cursor: "default" }}>{headerExtra}</span>}<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={titleColor || C.muted} strokeWidth="2" strokeLinecap="round" style={{ transform: o ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.3s ease", flexShrink: 0 }}><polyline points="6 9 12 15 18 9" /></svg></div>
    </button>
...
/* v256: the sandbox reaches the tool. The dashboard sets the flag and keeps it in localStorage under iva_sbx, and
   the tool runs on the same origin, so it reads the same key. In this mode DataForSEO answers from its sandbox,
   no credit is taken and no run, snapshot or report is written. */
async function ivaCoreRefund(ledgerId, reason) { if (!ledgerId) return null; try { const r = await fetch(SUPABASE_URL + "/functions/v1/credit-refund", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + (await ivaAuthToken()), "apikey": SUPABASE_KEY }, body: JSON.stringify({ ledger_id: ledgerId, reason: reason || "the run produced nothing" }) }); const d = await r.json(); if (d && d.ok) console.log("[IvaBot] credit returned:", d.refunded, "balance", d.balance); else console.warn("[IvaBot] the credit was not returned:", d && d.reason); return d; } catch(e) { console.warn("[IvaBot] refund call failed", e); return null; } }
const IVA_CORE_SBX = (function(){ try { if (/[?&]iva_sandbox=1/.test(location.search)) return true; if (/[?&]iva_sandbox=0/.test(location.search)) return false; return localStorage.getItem("iva_sbx") === "1"; } catch(e) { return false; } })();
if (IVA_CORE_SBX) { console.warn("[IvaBot] SANDBOX mode is on: DataForSEO answers are test data, no credit is charged and no row is written. Open the dashboard with ?iva_sandbox=0 to turn it off.");
  if (typeof window !== "undefined" && !window.__ivaSbxFetch) { window.__ivaSbxFetch = 1; const _ivaLiveFetch = window.fetch.bind(window);
    window.fetch = function(u, o) { try { if (String(u).indexOf("/functions/v1/dataforseo-proxy") >= 0 && o && typeof o.body === "string") { const b = JSON.parse(o.body); if (b && !b.sandbox) { b.sandbox = true; o = Object.assign({}, o, { body: JSON.stringify(b) }); } } } catch(e) { console.warn("[IvaBot] a request could not be marked as sandbox:", e); } return _ivaLiveFetch(u, o); };
    console.warn("[IvaBot] every DataForSEO call from this page now goes to the sandbox"); } }
const CORE_GPT = SUPABASE_URL + "/functions/v1/core-gpt";
const AIR_GPT = SUPABASE_URL + "/functions/v1/air-gpt"; /* migrated from Make WEBHOOK_URL (audit) + CHAT_WEBHOOK_URL (chat); old consts kept for rollback */
...
async function ivaKeepPageMarket(url, locale){
  try{
    if(!locale||!locale.location_code) return;
    var _sb=window.__supabase, _u=window.__userId||window.__memberId;
    if(!_sb||!_u) return;
    var key=String(url||"").toLowerCase().replace("https://","").replace("http://","");
    ...
    if(v[key]&&v[key].loc){ console.log("[IvaBot] this page already has a market, detection does not overwrite it:", key, v[key].loc); return; }
    v[key]={loc:locale.location_code,lang:(locale.language_code||"en"),setAt:new Date().toISOString(),by:"detected"};
    await _sb.from("dash_state").upsert({user_id:_u,key:"locale",value:v,updated_at:new Date().toISOString()},{onConflict:"user_id,key"});
    try{ localStorage.setItem("iva_dash_locale_"+_u, JSON.stringify(v)); }catch(_ls){}
    console.log("[IvaBot] market detected and kept for this page:", key, locale.location_code, locale.language_code);
  }catch(e){ console.warn("[IvaBot] keeping the page market failed", e); }
}
...
async function ivaManualLocale(dom){
  try{
    ...
    var q=await _sb.from("dash_state").select("value").eq("user_id",_u).eq("key","locale").limit(1);
    var v=(((q&&q.data)||[])[0]||{}).value||{};
    var _keys=[]; try{ var _pk=String(window.__ivaPageKey||"").toLowerCase(); if(_pk)_keys.push(_pk); }catch(_pe){}
    _keys.push(String(dom||"").toLowerCase().replace(/^www\./,""));
    for(var _i=0;_i<_keys.length;_i++){ var r=v[_keys[_i]]; if(r&&r.loc){ console.log("[IvaBot] market taken from the dashboard for", _keys[_i], r.loc, r.lang||"en"); return {location_code:r.loc,language_code:(r.lang||"en")}; } }
...
function detectLocale(url, htmlLang, hreflang, opts) {
  ...
  var countryToLoc = {
    ...
    aw:{loc:2533,lang:"en"}, cw:{loc:2531,lang:"en"}, bq:{loc:2535,lang:"en"},
...
function parseSEO(rawHtml, pageUrl) {
  ...
  r.title_slightly_long = r.title.length > 60 && r.title.length <= 80;
  r.title_too_long = r.title.length > 80;
  ...
  r.has_mobile = /<meta[^>]*name\s*=\s*["']?\s*viewport/i.test(html);
  ...
  let sc = 0;
  const ts = r.title_missing ? "missing" : r.title_too_short ? "too_short" : r.title_too_long ? "too_long" : (r.title_has_repeated_brand || r.title_has_duplicates) ? "duplicate" : "good";
...
function buildReportData(parsed, gpt, dfs) {
  ...
  return {
    ...
    titleEval: parsed.title_status !== "good" ? { why: parsed.title_missing ? "No meta title found — Google can't generate a proper search snippet." : (parsed.title_has_repeated_brand || parsed.title_has_duplicates) ? `Your title repeats words or the brand name ("${parsed.title}"). Repetition wastes characters that Google could use to describe what you offer. Use each word once and add terms for your services and location.` : parsed.title_too_short ? "Too short — users and Google need more context to understand what this page offers." : "Title is too long — Google will truncate it in search results.", sugLabel: "Suggested Titles", suggestions: (Array.isArray(gpt?.suggested_titles) && gpt.suggested_titles.length > 0) ? gpt.suggested_titles : (gpt?.suggested_title ? (Array.isArray(gpt.suggested_title) ? gpt.suggested_title : [gpt.suggested_title]) : (gpt?.keywords?.length > 0 ? gpt.keywords.map(k => k.charAt(0).toUpperCase() + k.slice(1) + " — " + (gpt?.page_context?.owner || parsed.hostname)) : ["Add a descriptive title with your primary keyword"])), showCopy: !!(Array.isArray(gpt?.suggested_titles) && gpt.suggested_titles.length > 0 || gpt?.suggested_title), links: [{ label: "Google Search Console", url: "https://search.google.com/search-console" }] } : null,
    descStatus: parsed.desc_status === "good" ? "good" : "bad",
    descEval: parsed.desc_status !== "good" ? { why: parsed.desc_missing ? "No meta description found — search engines can't generate a proper snippet." : parsed.desc_too_short ? "Description is too short — aim for 120-160 characters." : "Description is too long — Google may truncate it.", sugLabel: "Suggested Descriptions", suggestions: (Array.isArray(gpt?.suggested_descriptions) && gpt.suggested_descriptions.length > 0) ? gpt.suggested_descriptions : (gpt?.suggested_description ? (Array.isArray(gpt.suggested_description) ? gpt.suggested_description : [gpt.suggested_description]) : ["Write a 120-160 character description including your primary keyword"]), showCopy: !!(Array.isArray(gpt?.suggested_descriptions) && gpt.suggested_descriptions.length > 0 || gpt?.suggested_description) } : null,
...
const descCardTitle = (s) => s === "missing" ? "Meta Description Missing" : s === "too_short" ? "Meta Description Too Short" : s === "too_long" ? "Meta Description Too Long" : "Meta Description Needs Work";

function buildResults(d) {
  const NB = C.cardBorder;
  const g = [], b = [];
  if (d.titleStatus === "good") g.push({ title: "Meta Title", content: (<><SerpSnippet url={d.url} title={d.title} desc={d.desc} hideDesc /><BotNote inline text={`Your title is ${d.title.length} characters — right in the sweet spot (30–60). This is the #1 on-page signal Google uses to understand your content.`} /></>) }); else b.push({ ...d.titleEval, priority: "critical", title: d.titleEval?.title || titleCardTitle(d.titleStatus), serpSnippet: { url: d.url, title: d.title, desc: d.desc, hideDesc: true } });
  if (d.descStatus === "good") g.push({ title: "Meta Description", content: (<><SerpSnippet url={d.url} title={d.title} desc={d.desc} /><BotNote inline text={`Your description is ${d.desc.length} characters — within 120–160, the sweet spot. This is what users see in search results, so a good one means more clicks.`} /></>) }); else b.push({ ...d.descEval, priority: "critical", title: d.descEval?.title || descCardTitle(d.descStatus), serpSnippet: { url: d.url, title: d.title, desc: d.desc } });
  ...
  if (d.ux.cta.found) g.push({ title: "Call to Action", content: (<><InfoBlock label="Current CTA" value={`"${d.ux.cta.text}"`} borderColor={NB} /><BotNote inline text="A clear call-to-action guides visitors to the next step — buy, sign up, learn more. Pages with visible CTAs convert better." /></>) }); else b.push({ title: "No CTA Found", priority: "important", why: "A call-to-action (CTA) is a button or link that guides visitors to take action — like 'Buy now', 'Sign up', or 'Contact us'. Without one, visitors may leave without converting.", suggestions: ["Add a prominent CTA above the fold"], showCopy: false });
  if (d.ux.mobile) g.push({ title: "Mobile Optimization", content: (<><InfoBlock label="Status" value="Your page looks great on phones and tablets." borderColor={NB} /><BotNote inline text="Over 60% of searches happen on mobile. Google ranks mobile-friendly pages higher — and yours is ready." /></>) }); else b.push({ title: "Not Mobile-Friendly", priority: "critical", why: "Your page doesn't have a viewport meta tag, which means it won't display properly on phones and tablets. Google uses mobile-first indexing, so this directly hurts your rankings.", suggestions: ["Add viewport meta tag to your page head"], showCopy: false });
  ...
  if (!d.ux.noVideo) g.push({ title: "Video Content", content: (<><InfoBlock label="Status" value="Video detected on your page." borderColor={NB} /><BotNote inline text="Pages with video keep visitors engaged longer. Google notices this — it's a positive ranking signal." /></>) }); else b.push({...d.videoEval, priority: "nice"});
  if (d.speedStatus === "good") g.push({ title: "Page Speed", content: (<><InfoBlock label="Status" value="Your page loads fast." borderColor={NB} /><BotNote inline text="Speed is a Core Web Vital — Google uses it directly for rankings. Fast pages also have lower bounce rates and happier visitors." /></>) }); else b.push({...d.speedEval, priority: "important"});
...
async function generatePDF(data) {
  ...
  /* ══════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════ */
  const pB = [];
  if (data.titleStatus !== "good") pB.push({ t: data.titleEval?.title || titleCardTitle(data.titleStatus), p: "critical", w: data.titleEval?.why, s: data.titleEval?.suggestions, cur: data.title ? "Current title: \"" + data.title + "\" (" + data.title.length + " characters)" : "No title set" });
  if (data.descStatus !== "good") pB.push({ t: data.descEval?.title || descCardTitle(data.descStatus), p: "critical", w: data.descEval?.why, s: data.descEval?.suggestions, cur: data.desc ? "Current description: \"" + (data.desc.length > 120 ? data.desc.slice(0, 117) + "..." : data.desc) + "\" (" + data.desc.length + " characters)" : "No description set" });
  ...
  const allRecs = [
    ...
    { t: "Track your progress", p: "nice", w: "Re-run your audit on IvaBot right after making changes to see them reflected. Use Search Console and PageSpeed Insights monthly to track long-term progress.", s: [] }
  ];
...
const ReportV6 = ({ data, onNewAudit, onHome }) => { const { good, bad } = buildResults(data);
  ...
  <BotNote text="Let's start with where your page actually ranks in Google right now — real positions from the search index." />
  {data.rankedKeywords?.length > 0 && <div className="reveal reveal-delay-2" style={{ marginBottom: 16, borderRadius: 12, border: `1px solid ${C.cardBorder}`, background: C.surface }}>
  ...
  <div className="reveal reveal-delay-2" style={{ marginBottom: 20, borderRadius: 12, border: `1px solid ${C.cardBorder}`, background: C.surface }}>
    <div style={{ background: C.card, padding: "12px 16px", borderRadius: "12px 12px 0 0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
      <span style={{ display: "flex", alignItems: "center", gap: 5, minWidth: 0 }}><span style={{ fontSize: 14, fontWeight: 700, color: C.dark }}>What your page is built for</span><QM text="Based on your title, H1–H3 headings, and meta description — these are the search queries your content is currently optimized for." /></span>
      <span style={{ fontSize: 10, color: C.muted, background: "rgba(21,20,21,0.04)", padding: "3px 10px", borderRadius: 10, fontWeight: 500, flexShrink: 0 }}>content analysis</span>
    </div>
    <div style={{ padding: "2px 16px 12px" }}>
      <div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5, margin: "10px 0" }}>Based on your title, headings (H1–H3), and meta description — these are the search queries your content is currently optimized for.</div>
  ...
  <BotNote text="Now your backlinks — links from other websites to this page, one of Google's top ranking signals." />
  <div className="reveal" style={{ marginBottom: 16, borderRadius: 12, border: `1px solid ${C.cardBorder}`, background: C.surface }}>
    <div style={{ background: C.card, padding: "12px 16px", borderRadius: "12px 12px 0 0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
      <span style={{ display: "flex", alignItems: "center", gap: 5, minWidth: 0 }}><span style={{ fontSize: 14, fontWeight: 700, color: C.dark }}>Backlinks to this page</span><QM text="Links from other websites to this page — one of Google's top ranking signals. Track your full backlink profile over time in your dashboard." /></span>
      <DashLink href={_dashUrl} />
    </div>
    <div style={{ padding: "2px 16px 12px" }}>
  ...
  <div className="reveal" style={{ marginBottom: 8, padding: 20, borderRadius: 14, background: C.card, border: `1px solid ${C.cardBorder}` }}><div style={{ fontSize: 15, fontWeight: 700, color: C.dark, marginBottom: 12 }}>Final Recommendations</div><div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
    ...
    <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${(data.backlinksCount == null || data.backlinksCount < 10) ? "rgba(110,43,255,0.25)" : C.cardBorder}` }}><div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}><span style={{ color: C.accent, fontSize: 10, marginTop: 4 }}>●</span><div><div style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 2 }}>{data.backlinksCount != null && data.backlinksCount >= 10 ? "Keep building backlinks" : "Build quality backlinks — this is critical"}</div><div style={{ fontSize: 11.5, color: C.muted }}>{false && data.backlinksCount != null && data.backlinksCount < 10 ? `You currently have ${data.backlinksCount === 0 ? "no" : "only " + data.backlinksCount} backlink${data.backlinksCount !== 1 ? "s" : ""}. Backlinks are one of Google's top 3 ranking factors — without them, even perfect on-page SEO won't be enough. ` : ""}Reach out to industry blogs, directories, and publications. Offer guest posts, case studies, or resource mentions. Check the PR & Backlinks section above for specific opportunities.</div></div></div></div>
    ...
    <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}><span style={{ color: C.accent, fontSize: 10, marginTop: 4 }}>●</span><div><div style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 2 }}>Track your progress</div><div style={{ fontSize: 11.5, color: C.muted }}>Re-run your audit on IvaBot right after making changes to see them reflected. Use <a href="https://search.google.com/search-console" target="_blank" rel="noopener noreferrer" style={{ color: C.accent, textDecoration: "none" }}>Search Console</a> and <a href="https://pagespeed.web.dev/" target="_blank" rel="noopener noreferrer" style={{ color: C.accent, textDecoration: "none" }}>PageSpeed Insights</a> monthly to track long-term progress.</div></div></div></div>
  </div></div>
</div>); };
...
function CoreTool({ onHome }) {
  ...
  useEffect(() => { (async () => { const info = await getMemberInfo(); setMemberId(info.id); setMemberName(info.name); const cr = await fetchCredits(info.id); setCredits(cr); setLoading(false);
    ...
    if (savedCore && _shellIsReload()) {
      setTimeout(() => { setTool("core"); setView("chat"); sPLoad(null); setAuditData(savedCore); setSR(true); ivaAuditReadySignal(); setMsgs([{ from: "bot", content: "Here's your latest audit. Ask me anything about it, or start a New Audit.", id: Date.now() }]); }, 100);
    } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (_rep) {
        ...
        (async () => {
          try {
            const _rr = await fetch(SUPABASE_URL + "/rest/v1/reports?select=payload,created_at&id=eq." + encodeURIComponent(_rep) + "&flow_type=eq.core", {
              headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + (await ivaAuthToken()) }
            });
            ...
            if (_row && _row.payload) {
              setAuditData(_row.payload); setSR(true); ivaAuditReadySignal();
              const _rd = (function () { try { var d = new Date(_row.created_at); if (isNaN(d.getTime())) return ""; return d.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }); } catch (e) { return ""; } })(); console.log("[IvaBot] saved report of", _rd || "an unknown date"); setMsgs([{ from: "bot", content: (_rd ? ("Here's your saved " + "audit" + " from " + _rd + ". Ask me anything about it, or start a New Audit.") : "Here's your saved audit. Ask me anything about it, or start a New Audit."), id: Date.now() }]);
            } else {
  ...
  /* Reveal on scroll for report — staggered to prevent white screen */
  useEffect(() => {
    if (!showR) return;
  ...
  const runAudit = async (url) => {
    /* Charge up front: 1 credit, plus 1 for every 50 keywords added to this page. The count comes from dash_state, which the dashboard keeps, matched on the address without its protocol, www or trailing slash, since the two sides store it differently. The audit starts only on a confirmed charge: a wallet that answers ok:false, an HTTP error and a network failure all stop the run, because a run that goes ahead unpaid still costs money at DataForSEO. */
    let _paid = false, _why = "unavailable", _chargeId = null;
    try {
      ...
      if (IVA_CORE_SBX) { _paid = true; console.warn("[IvaBot] sandbox run, no credit was charged"); } else {
      const _chBody = _isU ? { p_user_id: memberId, p_action: "core_audit", p_cost: _cost, p_domain: url, p_page: url } : { p_member_id: memberId, p_action: "core_audit", p_cost: _cost, p_domain: url, p_page: url };
      ...
      if (_ch && _ch.ok === true) { _paid = true; _chargeId = _ch.ledger_id || null; }
      else if (_ch && _ch.ok === false) { _why = "insufficient"; }
      }
    } catch(e) { console.warn("[IvaBot] charge_credit core error:", e); }
    if (!_paid) {
      console.warn("[IvaBot] the audit did not start because the credit was not taken:", _why);
      try { sPLoad(null); } catch(e) {}
      addMsg("bot", React.createElement("div", null,
        React.createElement("div", {style:{marginBottom:6}}, _why === "insufficient" ? "Not enough credits to run this audit." : "The credit could not be taken just now, so the audit did not start. Try again in a moment."),
        _why === "insufficient" ? React.createElement("div", {style:{color:"#928E95",fontSize:12}}, React.createElement("button", {onClick:() => setSB(true), style:{color:"#6E2BFF",fontWeight:600,textDecoration:"underline",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:"inherit"}}, "Buy more credits"), " to continue.") : null
      ));
      return;
    }
    ...
    try {
      if (USE_MOCK) {
        ...
        try{ var _pk2=String(url||"").toLowerCase().replace("https://","").replace("http://",""); if(_pk2.charAt(_pk2.length-1)==="/")_pk2=_pk2.slice(0,-1); window.__ivaPageKey=_pk2; }catch(_pk3){ window.__ivaPageKey=""; }
        let locale = await ivaManualLocale(domain);
        if (locale) { console.log("[IvaBot] locale set by the user in the dashboard:", locale.location_code, locale.language_code); }
        else {
          var _mkOpts = { text: (((parsed.title||"")+" "+(parsed.body_text||"")).slice(0,8000)) };
          try{
            var _mr = await fetch(AIR_GPT, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + (await ivaAuthToken()) }, body: JSON.stringify({ step: "market", page_context: "URL: " + url + " | Title: " + (parsed.title||"") + " | Text: " + String(parsed.body_text||"").slice(0,4000) }) });
            var _md = _mr.ok ? await _mr.json() : null;
            var _mc = String((_md&&_md.country)||"").toLowerCase().trim();
            if(_mc && _mc !== "ww"){ _mkOpts.gptCountry = _mc; _mkOpts.gptLang = String((_md&&_md.language)||"").toLowerCase(); _mkOpts.gptConfidence = String((_md&&_md.confidence)||"").toLowerCase(); }
            console.log("[IvaBot] market read from the page:", _mc || "no answer", (_md&&_md.confidence)||"", (_md&&_md.why)||"");
          }catch(_me){ console.warn("[IvaBot] the market could not be read from the page, using the other signals", _me); }
          locale = detectLocale(url, parsed.html_lang, parsed.hreflang, _mkOpts);
          try{ await ivaKeepPageMarket(url, locale); }catch(_km){ console.warn("[IvaBot] the market could not be kept for this page", _km); }
        }
        ...
        try {
          ...
          /* v261: for an inner page the backlinks summary from DataForSEO answers for the whole domain, while the itemized list and the referring domains answer for the page itself. The proof was a run where the summary said 12,094 links and 58 domains while the list and the domains for the same target came back empty. The page therefore counts its own links from its own lists, and the summary is used only for the domain rank, which is a domain measure anyway. The home page keeps the summary, where the domain total is what people expect. */
          try {
            var _isInner = false;
            try { var _pu = new URL(url); _isInner = !!(_pu.pathname && _pu.pathname !== "/" && _pu.pathname !== ""); } catch (_ip) {}
            try { window.__ivaInnerRun = _isInner; } catch (_if) {}
            if (_isInner) {
              var _blOwn = (dfsSeo.backlinksList || []).filter(function (b) { return b && b.kind !== "domain"; }).length;
              var _rdOwn = (dfsSeo.referringDomainsList || []).length;
              dfsSeo.backlinksCount = _blOwn;
              dfsSeo.referringDomains = _rdOwn;
              console.log("[IvaBot] inner page: backlinks counted from this page's own lists —", _blOwn, "links,", _rdOwn, "referring domains");
            }
          } catch (_pb) { console.warn("[IvaBot] could not count the page's own backlinks:", _pb); }
      ...
      /* v256: tell the shell that the finished audit is on screen, so the Dashboard link in the
         nav starts breathing. The signal is sent here rather than after the database writes,
         because the reader sees the report at this moment and a failed write must not silence it. */
      ivaAuditReadySignal();
      try { saveCoreReport(memberId, reportData); } catch(e){}
      if (IVA_CORE_SBX) console.warn("[IvaBot] sandbox run, no run row, snapshot or report is written");
      if (!USE_MOCK && !IVA_CORE_SBX) {
        ...
        try {
          ...
          const snapBody = {
            ...
            p_backlinks_count: ((reportData.backlinksCount === 0 || reportData.backlinksCount) ? reportData.backlinksCount : (function(){ try{ var _p9=new URL(url).pathname.replace(/\/$/,""); return (_p9===""||_p9==="/")?null:0; }catch(_e9){ return null; } })()),
            p_referring_domains_count: ((reportData.referringDomains === 0 || reportData.referringDomains) ? reportData.referringDomains : (function(){ try{ var _p9=new URL(url).pathname.replace(/\/$/,""); return (_p9===""||_p9==="/")?null:0; }catch(_e9){ return null; } })()),
            ...
            p_coverage: (function(){ try {
              ...
              function _w(ev, fb){ try { var t = (ev && (ev.suggestions && ev.suggestions[0])) || (ev && ev.why) || fb || ""; return String(t).replace(/\s+/g, " ").trim(); } catch(e) { return fb || ""; } }
              ...
              if (R.titleStatus !== "good") B.push({ title: (R.titleEval && R.titleEval.title) || tt, why: _w(R.titleEval, "Write a title of 30 to 60 characters with the main keyword near the front."), priority: "critical" });
              if (R.descStatus !== "good") B.push({ title: (R.descEval && R.descEval.title) || dt, why: _w(R.descEval, "Write a description of 120 to 160 characters that gives a reason to click."), priority: "critical" });
              if (R.headingsStatus !== "good") B.push({ title: "Heading Structure", why: _w(R.headingsEval, "Use one H1 that states what the page is about, then H2s for the sections under it."), priority: "critical" });
              if (!(ux.mobile)) B.push({ title: "Not Mobile-Friendly", why: "Add a viewport meta tag so the page renders properly on phones.", priority: "critical" });
              if (R.linksStatus !== "good") B.push({ title: "Links", why: _w(R.linksEval, "Link to your other relevant pages, so visitors and search engines can find them."), priority: "important" });
              if (!(ux.cta && ux.cta.found)) B.push({ title: "No CTA Found", why: "Add a clear button or link telling visitors what to do next.", priority: "important" });
              if (R.speedStatus !== "good") B.push({ title: (R.speedEval && R.speedEval.title) || "Page Speed", why: _w(R.speedEval, "Compress images and cut unused scripts to bring load time down."), priority: "important" });
              if (ux.altMissing) B.push({ title: (R.imagesEval && R.imagesEval.title) || "Images Missing Alt", why: _w(R.imagesEval, "Describe each image in its alt text, so search engines and screen readers understand it."), priority: "nice" });
              if (ux.noVideo) B.push({ title: (R.videoEval && R.videoEval.title) || "No Video", why: _w(R.videoEval, "Add a short video where it helps: pages with video hold attention longer."), priority: "nice" });
              if (R.robotsStatus !== "good") B.push({ title: "robots.txt Not Found", why: "Add a robots.txt at the site root telling search engines what to crawl.", priority: "nice" });
              if (R.sitemapStatus !== "good") B.push({ title: "Sitemap Not Found", why: "Publish an XML sitemap so Google finds every page faster.", priority: "nice" });
      ...
      setLS(-1);
      sPLoad(null);
      /* v266: the credit is taken before the work, so a run that ends here leaves the person paying for nothing.
         The charge is returned by its ledger id, which the refund function accepts once and only from its owner. */
      if (_chargeId) { try { await ivaCoreRefund(_chargeId, "Core Audit stopped: " + (err && err.message ? err.message : "unknown error")); } catch (_re) { console.warn("[IvaBot] refund attempt failed", _re); } }
      addMsg("bot", `Something went wrong: ${err.message}.${_chargeId ? " The credit was returned." : ""} Please try again.`, true);
    }
  };
    ...
    <React.Fragment>
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          {showR && <div style={{ display: "flex", gap: 8, flexWrap: "wrap", padding: isMobile ? "8px 12px 16px" : "8px 24px 16px", maxWidth: isMobile ? "100%" : 1224, margin: "0 auto", width: "100%", alignItems: "center" }}>
            ...
            <button id="export-pdf-btn" onClick={() => generatePDF(auditData)} style={{ height: 40, padding: "0 20px", borderRadius: 10, background: C.surface, border: `1px solid ${C.borderMid}`, color: C.dark, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", display: "inline-flex", alignItems: "center", gap: 6, boxSizing: "border-box" }} onMouseEnter={e => e.currentTarget.style.background = C.accentLight} onMouseLeave={e => e.currentTarget.style.background = C.surface}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>Export PDF</button>
