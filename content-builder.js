/* IvaBot Content Builder v74 — replaced the "Try Another Tool" button with the "Dashboard" button (links to https://ivabot.xyz/dashboard, opens in new tab), matching Core Audit and AI Readiness. v73 — brand guide upload (read file, 15K limit, send to GPT) */
(function() {
...
/* HL prep — shared between hlF (React) and hlHtml (string) */
...
/* v67b: Exact + Partial keyword highlighting — NO nested marks. */
    ...
    (async()=>{
      try{
        ...
        if(!crd.ok){
          ...
          bot(<div><div style={{marginBottom:6}}>You have <strong>{crd.limit-crd.used}</strong> Content Builder credits remaining.</div><div style={{color:C.muted,fontSize:12,marginBottom:8}}>Starting a new content session will use 1 credit.</div><div style={{display:"flex",gap:8}}><Btn text="Start New Content" primary onClick={()=>{add("u","Start new");clearBuilderReport(getMemberId());setStep("ec");sRp(null);sBd(null);sStit("");confirmedTitleRef.current="";sAns({});sKwData([]);skwRef.current=[];sConfirmedKw([]);dfsExtraRef.current={};sMsgs([]);sPLoad(null);setContentHtml("");bot(<div><div style={{marginBottom:6}}>Hey! Let's build new SEO content.</div><div style={{fontWeight:600,marginBottom:6}}>Do you have keywords or should I find them?</div></div>);}}/><Btn text="Stay Here" onClick={()=>{bot("No problem! You can keep editing your current content.");}}/></div></div>);
        }
