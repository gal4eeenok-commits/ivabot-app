...
console.log("[IvaBot] seo-tools.js v69 loaded");
...
const Tip = ({ text, children }) => { const [s, setS] = useState(false); const ref = useRef(null); const [pos, setPos] = useState({ top: 0, left: 0, above: true }); return (<span ref={ref} style={{ position: "relative", display: "inline-flex", alignItems: "center" }} onMouseEnter={() => { if (ref.current) { const rect = ref.current.getBoundingClientRect(); const above = rect.top > 180; const alignRight = rect.left > window.innerWidth - 280; setPos({ top: above ? rect.top - 8 : rect.bottom + 8, left: alignRight ? Math.max(8, rect.right - 260) : Math.max(8, rect.left), above }); } setS(true); }} onMouseLeave={() => setS(false)}>{children}{s && <span style={{ position: "fixed", top: pos.above ? "auto" : pos.top, bottom: pos.above ? (window.innerHeight - pos.top) + "px" : "auto", left: pos.left, background: C.surface, color: C.dark, padding: "10px 14px", borderRadius: 10, fontSize: 11, lineHeight: 1.5, width: 260, maxWidth: "85vw", zIndex: 9999, fontWeight: 400, boxShadow: "0 4px 24px rgba(0,0,0,0.14)", border: `1px solid ${C.border}`, pointerEvents: "none", whiteSpace: "normal", wordBreak: "break-word", textAlign: "left" }}>{text}</span>}</span>); };
...
const BB = ({children}) => <div className="iva-msg-in" style={{display:"flex",flexDirection:"column",alignItems:"flex-start",maxWidth:"90%",alignSelf:"flex-start"}}><div style={{marginBottom:3,marginLeft:2}}><BL s={16}/></div><div style={{padding:"10px 14px",borderRadius:"4px 12px 12px 12px",background:C.surface,border:`1px solid ${C.border}`,fontSize:13,color:C.dark,lineHeight:1.5}}>{children}</div></div>;
const UB = ({children,n}) => <div className="iva-msg-in" style={{display:"flex",flexDirection:"column",alignItems:"flex-end",maxWidth:"80%",alignSelf:"flex-end"}}><div style={{marginBottom:3,marginRight:2}}><UA n={n}/></div><div style={{padding:"8px 14px",borderRadius:"12px 4px 12px 12px",background:C.accent,fontSize:13,color:"#fff"}}>{children}</div></div>;
...
function IvaBotV6() {
  ...
  const PanelContent = () => <React.Fragment>
    ...
    {!pLoad && showR && auditData && <div style={{ animation: "fadeIn 0.5s ease", minHeight: "calc(100vh - 130px)" }}><ReportV6 data={auditData} onNewAudit={() => { setSR(false); setMsgs([]); setLS(-1); setAuditData(null); sPLoad(null); }} onHome={home} /></div>}
