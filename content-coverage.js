/* IvaBot Content Coverage v6.3 ÃÂ¢ÃÂÃÂ SERP positions all keywords, PDF safe symbols, Working Well open */
(function() {
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ MEMBER ID + CREDITS ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ COLORS (identical to Core Audit + CB) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ PRIMITIVES (1:1 from seo-tools.js) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
...
const RankingsTable=({rows,emptyMsg})=>(<div style={{background:C.surface,borderRadius:10,padding:"4px 14px",border:`1px solid ${C.cardBorder}`,overflow:"visible"}}><table style={{width:"100%",minWidth:380,borderCollapse:"collapse",fontSize:12.5}}><thead><tr style={{borderBottom:`1px solid ${C.border}`}}><th style={{textAlign:"left",padding:"8px 0",color:C.muted,fontWeight:500,fontSize:11.5}}>Keyword</th><th style={{textAlign:"center",padding:"8px 4px",color:C.muted,fontWeight:500,fontSize:11.5,width:50,whiteSpace:"nowrap"}}>Pos. <QM text="Your page's position in Google search results for this keyword. Position 1 = top result. Based on your target region."/></th><th style={{textAlign:"right",padding:"8px 4px",color:C.muted,fontWeight:500,fontSize:11.5,width:70,whiteSpace:"nowrap"}}>Vol. <QM text="Monthly search volume ÃÂ¢ÃÂÃÂ how many times per month people search this keyword in Google."/></th><th style={{textAlign:"right",padding:"8px 0",color:C.muted,fontWeight:500,fontSize:11.5,width:50,whiteSpace:"nowrap"}}>KD <QM text="Keyword difficulty (0ÃÂ¢ÃÂÃÂ100) ÃÂ¢ÃÂÃÂ how hard it is to rank in the top 10. Under 30 = easy, 30ÃÂ¢ÃÂÃÂ60 = moderate, 60+ = hard."/></th></tr></thead><tbody>{rows&&rows.length>0?rows.map((r,i)=>(<tr key={i} style={{borderBottom:i<rows.length-1?`1px solid rgba(21,20,21,0.04)`:"none"}}><td style={{padding:"10px 8px 10px 0",color:C.dark,fontWeight:500}}>{r.keyword}</td><td style={{textAlign:"center",padding:"10px 4px"}}>{r.position!=null?(<span style={{background:r.position<=3?"rgba(110,43,255,0.08)":"rgba(21,20,21,0.04)",color:r.position<=3?C.accent:C.muted,fontWeight:600,padding:"3px 10px",borderRadius:8,fontSize:12}}>{r.position}</span>):<Tip text="Position data unavailable ÃÂ¢ÃÂÃÂ this keyword may be too niche for automated tracking. Your page could still rank for it."><span style={{color:C.muted,fontSize:10,background:"rgba(21,20,21,0.03)",padding:"3px 8px",borderRadius:6,cursor:"help"}}>ÃÂ¢ÃÂÃÂ</span></Tip>}</td><td style={{textAlign:"right",padding:"10px 4px",whiteSpace:"nowrap"}}>{r.volume!=null&&r.volume>0?<span style={{color:C.dark,fontSize:12}}>{fmtVol(r.volume)}</span>:<NicheBadge/>}</td><td style={{textAlign:"right",padding:"10px 0",whiteSpace:"nowrap"}}>{r.difficulty!=null?<span style={{color:C.muted,fontSize:12}}>{r.difficulty}</span>:<LowBadge/>}</td></tr>)):<tr><td colSpan={4} style={{padding:"14px 0",color:C.muted,fontSize:12,textAlign:"center"}}>{emptyMsg||"No data available yet."}</td></tr>}</tbody></table></div>);
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ HTML PARSER ÃÂ¢ÃÂÃÂ extended from Core Audit parseSEO ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
function parseCoverage(rawHtml, pageUrl) {
  ...
  // Body text (paragraphs only ÃÂ¢ÃÂÃÂ for keyword density)
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ BODY KEYWORD DENSITY ÃÂ¢ÃÂÃÂ ported from Typebot single_body_density.js ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
function analyzeBodyDensity(bodyText, keywords) {
  ...
  const STOP_RU = new Set(["ÃÂÃÂ¸","ÃÂÃÂ²","ÃÂÃÂ½ÃÂÃÂ°","ÃÂÃÂ¿ÃÂÃÂ¾","ÃÂÃÂº","ÃÂÃÂ","ÃÂÃÂ","ÃÂÃÂ¾","ÃÂÃÂ¾ÃÂÃÂ","ÃÂÃÂ´ÃÂÃÂ¾","ÃÂÃÂ·ÃÂÃÂ°","ÃÂÃÂ¸ÃÂÃÂ·","ÃÂÃÂ²ÃÂÃÂ¾","ÃÂÃÂ½ÃÂÃÂµ","ÃÂÃÂ´ÃÂÃÂ°","ÃÂÃÂ½ÃÂÃÂ¾","ÃÂÃÂ¸ÃÂÃÂ»ÃÂÃÂ¸","ÃÂÃÂ¶ÃÂÃÂµ","ÃÂÃÂÃÂÃÂÃÂÃÂ¾","ÃÂÃÂÃÂÃÂ°ÃÂÃÂº","ÃÂÃÂºÃÂÃÂ°ÃÂÃÂº","ÃÂÃÂ´ÃÂÃÂ»ÃÂÃÂ","ÃÂÃÂ¿ÃÂÃÂÃÂÃÂ¸","ÃÂÃÂ½ÃÂÃÂ°ÃÂÃÂ´","ÃÂÃÂ±ÃÂÃÂµÃÂÃÂ·"]);
  ...
  const PREPS = new Set(["to","in","for","from","during","of","on","at","with","by","about","and","the","a","an","ÃÂÃÂ²","ÃÂÃÂ½ÃÂÃÂ°","ÃÂÃÂ´ÃÂÃÂ»ÃÂÃÂ","ÃÂÃÂ¿ÃÂÃÂ¾","ÃÂÃÂ","ÃÂÃÂ¸ÃÂÃÂ·","ÃÂÃÂ¾ÃÂÃÂ","ÃÂÃÂ´ÃÂÃÂ¾","ÃÂÃÂ·ÃÂÃÂ°","ÃÂÃÂ¾","ÃÂÃÂº"]);
  ...
  for (const k of keywords) {
    ...
    if (exact > 0) occurrences.push(`ÃÂ¢ÃÂÃÂ¢ phrase "${k.toLowerCase()}" ÃÂ¢ÃÂÃÂ appears ${exact} ${exact===1?"time":"times"}`);
    else if (soft > 0) occurrences.push(`ÃÂ¢ÃÂÃÂ¢ phrase "${k.toLowerCase()}" ÃÂ¢ÃÂÃÂ appears ${soft} ${soft===1?"time":"times"} (with prepositions)`);
  }
  // Individual words
  ...
  for (const t of tokens) {
    ...
    if (c > 0) occurrences.push(`ÃÂ¢ÃÂÃÂ¢ word "${t}" ÃÂ¢ÃÂÃÂ appears ${c} ${c===1?"time":"times"}`);
  }
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ KEYWORD VALIDATION ÃÂ¢ÃÂÃÂ clean user input ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
function validateUserKeywords(raw) {
  ...
  for (const p of parts) {
    ...
    if (words.length < 2) { errors.push(`"${p}" ÃÂ¢ÃÂÃÂ needs at least 2 words`); continue; }
    if (words.length > 4) { errors.push(`"${p}" ÃÂ¢ÃÂÃÂ max 3-4 words per phrase`); continue; }
    cleaned.push(p);
  }
  return { keywords: cleaned.slice(0, 3), errors, valid: cleaned.length > 0 };
}

/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ TITLE/DESC/HEADINGS KEYWORD ANALYSIS ÃÂ¢ÃÂÃÂ ported from Typebot ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
function analyzeTitleDescHeadings(parsed, keywords) {
  const STOP = new Set(["a","an","the","and","or","but","of","in","on","to","for","from","with","as","is","are","was","were","be","been","being","it","its","at","by","if","not","no","so","than","that","this","into","up","down","out","off","via","per","about","can","could","should","would","do","does","did","have","has","had","you","your","we","our","they","their","he","she","him","her","his","them","i","me","my","ÃÂÃÂ¸","ÃÂÃÂ²","ÃÂÃÂ½ÃÂÃÂ°","ÃÂÃÂ¿ÃÂÃÂ¾","ÃÂÃÂº","ÃÂÃÂ","ÃÂÃÂ","ÃÂÃÂ¾","ÃÂÃÂ¾ÃÂÃÂ","ÃÂÃÂ´ÃÂÃÂ¾","ÃÂÃÂ·ÃÂÃÂ°","ÃÂÃÂ¸ÃÂÃÂ·","ÃÂÃÂ²ÃÂÃÂ¾","ÃÂÃÂ½ÃÂÃÂµ","ÃÂÃÂ´ÃÂÃÂ°","ÃÂÃÂ½ÃÂÃÂ¾","ÃÂÃÂ¸ÃÂÃÂ»ÃÂÃÂ¸","ÃÂÃÂ¶ÃÂÃÂµ","ÃÂÃÂÃÂÃÂÃÂÃÂ¾","ÃÂÃÂºÃÂÃÂ°ÃÂÃÂº","ÃÂÃÂ´ÃÂÃÂ»ÃÂÃÂ","ÃÂÃÂ¿ÃÂÃÂÃÂÃÂ¸"]);
  ...
  result.title = {
    ...
    tooLong: titleLen > 90,
    occurrences: titleKwResults.map(r => `ÃÂ¢ÃÂÃÂ¢ "${r.keyword}" ÃÂ¢ÃÂÃÂ ${r.label === "exact" ? `exact match (${r.exact}ÃÂÃÂ)` : r.label === "all_words" ? "all words present (not exact)" : r.label === "some_words" ? "some words present" : "not found"}`).join("\n"),
    // Rank booleans for full balance
  ...
  result.h1 = {
    ...
    strong: h1HasExact,
    occurrences: h1KwResults.map(r => `ÃÂ¢ÃÂÃÂ¢ "${r.keyword}" ÃÂ¢ÃÂÃÂ ${r.label === "exact" ? `found in H1` : r.label === "all_words" ? "all words present" : r.label === "some_words" ? "some words" : "not in H1"}`).join("\n"),
  };
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ SEMANTIC FILTER ÃÂ¢ÃÂÃÂ from Typebot single_semantic.js ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ HEADING LIST (from Core Audit ÃÂ¢ÃÂÃÂ colored badges) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ COVERAGE SCORE CARD ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ BUILD RESULTS (same structure as v4 but with real data) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
function buildCoverageResults(d) {
  ...
  /* Title ÃÂ¢ÃÂÃÂ with keyword presence analysis */
  ...
  if (ta) {
    ...
    if (titleOk) {
      ...
      const whyParts = [];
      if (ta.missing) whyParts.push("No meta title found ÃÂ¢ÃÂÃÂ Google can't generate a proper search snippet.");
      else {
        if (ta.tooShort) whyParts.push(`Title is only ${ta.length} characters ÃÂ¢ÃÂÃÂ too short. Aim for 30ÃÂ¢ÃÂÃÂ60 characters.`);
        if (ta.tooLong) whyParts.push(`Title is ${ta.length} characters ÃÂ¢ÃÂÃÂ too long, Google will truncate it.`);
        if (ta.coverage === "none") whyParts.push("None of your target keywords appear in the title ÃÂ¢ÃÂÃÂ this is the #1 on-page SEO signal.");
        else if (ta.coverage === "partial") whyParts.push("Your keywords are partially present in the title, but not as exact phrases.");
        if (ta.duplicates.length) whyParts.push("Duplicate words found: " + ta.duplicates.map(([w,c]) => `"${w}" (${c}ÃÂÃÂ)`).join(", ") + ". Rephrase to avoid repetition.");
        if (ta.overused) whyParts.push("Keywords are overused in the title ÃÂ¢ÃÂÃÂ keep each phrase to 1 mention.");
      }
      const titlePrio = (d.pageType === "about" || d.pageType === "contact") ? "important" : "critical";
      contentBad.push({ title: "Meta Title Needs Work", priority: titlePrio, serpSnippet: { url: d.url, title: d.title || "(no title)", desc: d.desc, hideDesc: true }, currentLabel: "Keyword presence in title", current: ta.occurrences, why: whyParts.join(" "), suggestions: d.gptSuggestions?.suggested_titles?.length > 0 ? d.gptSuggestions.suggested_titles : ["Add your primary keyword phrase once near the beginning of the title", "Keep title between 30ÃÂ¢ÃÂÃÂ60 characters"], showCopy: !!(d.gptSuggestions?.suggested_titles?.length > 0) });
    }
  ...
  /* Description ÃÂ¢ÃÂÃÂ with keyword check */
  const da = d.titleAnalysis?.desc;
  if (da) {
    if (da.good && da.hasKeywords) {
      contentGood.push({ title: "Meta Description", content: (<><SerpSnippet url={d.url} title={d.title} desc={d.desc} /><BotNote inline text={`Your description is ${da.length} characters ÃÂ¢ÃÂÃÂ within 90ÃÂ¢ÃÂÃÂ180, with keyword mentions. Good for click-through rate.`} /></>) });
    } else {
      const whyParts = [];
      if (da.missing) whyParts.push("No meta description found.");
      else {
        if (da.tooShort) whyParts.push(`Description is only ${da.length} characters ÃÂ¢ÃÂÃÂ too short. Aim for 140ÃÂ¢ÃÂÃÂ160 characters.`);
        if (da.tooLong) whyParts.push(`Description is ${da.length} characters ÃÂ¢ÃÂÃÂ too long, Google may truncate it.`);
  ...
  /* Headings ÃÂ¢ÃÂÃÂ with keyword presence */
  ...
  if (h1a && h2h3a) {
    ...
    if (h1Ok && h2h3Ok) {
      ...
      contentGood.push({ title: "Heading Structure", content: (<><InfoBlock label={`H1 ÃÂ¢ÃÂÃÂ ${h1.length} found`} value={<HL tags={h1} lv="H1" />} borderColor={NB} /><InfoBlock label={`H2 ÃÂ¢ÃÂÃÂ ${h2.length} found`} value={<HL tags={h2} lv="H2" />} borderColor={NB} />{h3.length > 0 && <InfoBlock label={`H3 ÃÂ¢ÃÂÃÂ ${h3.length} found`} value={<HL tags={h3} lv="H3" />} borderColor={NB} />}<InfoBlock label="Keyword presence" value={h1a.occurrences} borderColor={NB} /><BotNote inline text="Your heading structure is well-organized with keywords present. Google uses this hierarchy to understand your page." /></>) });
    } else {
      if (h1a.missing || h1a.matchesTitle || h1a.tooLong || !h1a.strong) {
        const h1WhyParts = [];
        if (h1a.missing) h1WhyParts.push("H1 is missing ÃÂ¢ÃÂÃÂ this is your page's main heading. Google uses it to understand what the page is about.");
        if (h1a.tooLong) h1WhyParts.push(`H1 is ${h1a.list[0]?.length || 0} characters ÃÂ¢ÃÂÃÂ way too long. Keep H1 under 60ÃÂ¢ÃÂÃÂ80 characters for best impact.`);
        if (h1a.matchesTitle) h1WhyParts.push("H1 is identical to the title ÃÂ¢ÃÂÃÂ rewrite it to expand on the title while keeping your primary keyword.");
        if (!h1a.strong && !h1a.missing) h1WhyParts.push("Your target keywords are not in the H1 heading.");
        const h1Why = h1WhyParts.join(" ");
        contentBad.push({ title: "H1 Needs Work", priority: "critical", currentLabel: "Current H1", current: d.h1.length > 0 ? <HL tags={d.h1} lv="H1" /> : "No H1 found", why: h1Why, suggestions: d.gptSuggestions?.suggested_h1?.length > 0 ? d.gptSuggestions.suggested_h1 : ["Add a unique H1 with your primary keyword", "Make it different from the title ÃÂ¢ÃÂÃÂ expand on the topic"], showCopy: !!(d.gptSuggestions?.suggested_h1?.length > 0) });
      }
      if (h2h3a.h2Count === 0 || !h2h3a.hasKeywords) {
        const h2Why = h2h3a.h2Count === 0 ? "No H2 headings found ÃÂ¢ÃÂÃÂ add subheadings to break content into sections." : "Keywords are missing from H2/H3 headings. Include secondary keywords in subheadings.";
  ...
  if (d.bodyEval) {
    const be = d.bodyEval;
    if (be.status === "good") {
      contentGood.push({ title: "Body Content ÃÂ¢ÃÂÃÂ Keyword Coverage", content: (<><InfoBlock label="Keyword scan results" value={be.occurrences} borderColor={NB} /><BotNote inline text="Your keywords appear naturally throughout the body text ÃÂ¢ÃÂÃÂ good balance without overuse." /></>) });
    } else {
      const kwList = (ukw.length ? ukw : d.extractedKeywords || []);
      const actionLines = kwList.map(k => `"${typeof k === "string" ? k : k.keyword}" ÃÂ¢ÃÂÃÂ ${be.budgetPer[0]}ÃÂ¢ÃÂÃÂ${be.budgetPer[1]} times`);
      contentBad.push({ title: "Body Content ÃÂ¢ÃÂÃÂ Keyword Coverage", priority: "critical", currentLabel: "Keyword scan results", current: be.occurrences, why: be.status === "no_body" ? "No body text detected on your page." : be.status === "missing" ? "Body content has individual keyword words but no exact phrases. Google weights exact phrase matches more heavily." : be.status === "low" ? "Partial coverage ÃÂ¢ÃÂÃÂ add at least one more full key phrase." : "Keyword usage appears high ÃÂ¢ÃÂÃÂ reduce repetition.", sugLabel: "Recommended keyword usage", suggestions: [...actionLines, `Total recommended: ${be.budgetTotal[0]}ÃÂ¢ÃÂÃÂ${be.budgetTotal[1]} times across the entire body text.`], showCopy: false });
    }
  ...
  if (d.semanticMissing) {
    ...
    if (allMissing.length > 0) {
      contentBad.push({ title: "Semantic Expansion ÃÂ¢ÃÂÃÂ Topics You May Be Missing", priority: "nice", soft: true,
        why: "I checked real Google search data and found topics related to your keywords that are missing from your page. Adding some of these phrases can help your page appear for more search queries and bring in more targeted visitors. Only add phrases that naturally fit your content ÃÂ¢ÃÂÃÂ don't force them in.",
        currentLabel: "Missing from your page",
        current: (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {sm.autocomplete.length > 0 && <div><div style={{ fontSize: 11, fontWeight: 600, color: C.dark, marginBottom: 2 }}>Autocomplete <QM text="These are Google's suggestions that appear as users type your keyword into the search bar. They reflect real, popular search patterns." /></div><div style={{ fontSize: 11.5, color: C.muted, marginBottom: 6 }}>Google's suggestions as you type ÃÂ¢ÃÂÃÂ real, popular search patterns.</div>{sm.autocomplete.map((s, i) => <div key={i} style={{ fontSize: 12, color: C.dark, padding: "3px 0" }}>ÃÂ¢ÃÂÃÂ¢ {s}</div>)}</div>}
          {sm.related.length > 0 && <div><div style={{ fontSize: 11, fontWeight: 600, color: C.dark, marginBottom: 2 }}>Related searches <QM text="These queries appear at the bottom of Google's search results page. They show what users also search for after your keyword." /></div><div style={{ fontSize: 11.5, color: C.muted, marginBottom: 6 }}>Queries users also look for ÃÂ¢ÃÂÃÂ shown at the bottom of Google results.</div>{sm.related.map((s, i) => <div key={i} style={{ fontSize: 12, color: C.dark, padding: "3px 0" }}>ÃÂ¢ÃÂÃÂ¢ {s}</div>)}</div>}
          {sm.paa.length > 0 && <div><div style={{ fontSize: 11, fontWeight: 600, color: C.dark, marginBottom: 2 }}>People Also Ask <QM text="These are real questions that Google shows in a special FAQ-style block on the search results page. Answering them on your page can help you appear in this block." /></div><div style={{ fontSize: 11.5, color: C.muted, marginBottom: 6 }}>Common questions shown in Google ÃÂ¢ÃÂÃÂ answering them can get you featured.</div>{sm.paa.map((s, i) => <div key={i} style={{ fontSize: 12, color: C.dark, padding: "3px 0" }}>ÃÂ¢ÃÂÃÂ¢ {s}</div>)}</div>}
        </div>),
        sugLabel: "How to use these phrases",
        suggestions: [...(sm.related.length > 0 ? ["Related Searches ÃÂ¢ÃÂÃÂ pick 1ÃÂ¢ÃÂÃÂ2 phrases and use them as H2 or H3 headings, or mention them in your body text."] : []), ...(sm.paa.length > 0 ? ["People Also Ask ÃÂ¢ÃÂÃÂ take 3ÃÂ¢ÃÂÃÂ5 questions and add a short FAQ section to your page. This helps you appear in Google's FAQ block."] : []), ...(sm.autocomplete.length > 0 ? ["Autocomplete ÃÂ¢ÃÂÃÂ mention 1ÃÂ¢ÃÂÃÂ2 of these phrases naturally in your text where they fit the context. Don't overuse them."] : [])],
        showCopy: false
      });
    }
  }
  ...
  if (d.trust) {
    const t = d.trust;
    if (t.contacts.show) {
      if (t.contacts.found) trustGood.push({ title: "Contact Information", content: (<><InfoBlock label="Status" value="Contact information detected on your page." borderColor={NB} /><BotNote inline text="Visible contact info builds trust with visitors and search engines ÃÂ¢ÃÂÃÂ it's a key E-E-A-T signal." /></>) });
    ...
    if (t.testimonials.show) {
      ...
      else trustBad.push({ title: "No Testimonials Found", priority: "nice", why: "No testimonials or reviews detected. Adding 2ÃÂ¢ÃÂÃÂ3 short customer quotes builds social proof.", suggestions: ["Add 2ÃÂ¢ÃÂÃÂ3 short testimonials from customers", "Include star ratings or review counts if available"], showCopy: false });
    }
    if (t.faq.show) {
      ...
      else trustBad.push({ title: "No FAQ Section", priority: "nice", why: "No FAQ section found. Adding 3ÃÂ¢ÃÂÃÂ5 common questions improves E-E-A-T signals.", suggestions: ["Add a short FAQ with 3ÃÂ¢ÃÂÃÂ5 questions related to your topic"], showCopy: false });
    }
  }
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ RANKING GAPS ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
function buildRankingGaps(d) {
  const g = [];
  if (d.titleStatus !== "good") g.push({ text: "Title needs work ÃÂ¢ÃÂÃÂ lacks descriptive keywords and context.", critical: true });
  if (d.h1Status !== "good") g.push({ text: "H1 too short or matches the title ÃÂ¢ÃÂÃÂ rewrite for clarity.", critical: true });
  if (d.bodyStatus !== "good") g.push({ text: "Missing keyword phrases in body text ÃÂ¢ÃÂÃÂ weak search relevance.", critical: true });
  if (d.trust && !d.trust.testimonials?.found && d.trust.testimonials?.show) g.push({ text: "No testimonials ÃÂ¢ÃÂÃÂ weak social proof.", critical: false });
  if (d.trust && !d.trust.faq?.found && d.trust.faq?.show) g.push({ text: "No FAQ section ÃÂ¢ÃÂÃÂ fewer trust indicators.", critical: false });
  if (d.trust && !d.trust.socials?.found && d.trust.socials?.show) g.push({ text: "Missing social profiles ÃÂ¢ÃÂÃÂ weak off-page trust.", critical: false });
  return g;
}

/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ REPORT ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
const CoverageReport = ({ data }) => {
  ...
  return (<div style={{ maxWidth: 580, margin: "0 auto", padding: "20px 16px 16px" }}>
    <BotNote text="Here's your full content coverage audit. I'll walk you through each part ÃÂ¢ÃÂÃÂ what's working, what needs fixing, and exactly how to fix it." />
    ...
    {(() => {
      ...
      return (<>
        ...
        <div className="reveal reveal-delay-2" style={{ marginBottom: 16, padding: 16, borderRadius: 12, background: C.card, border: `1px solid ${C.cardBorder}` }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 13, fontWeight: 700, color: C.dark }}>What your page is built for</span><QM text="Based on your title, H1ÃÂ¢ÃÂÃÂH3 headings, and meta description." /></div>
    ...
    {contentBad.length > 0 && <div className="reveal" style={{ marginBottom: 20 }}><Fold title="Content & Structure ÃÂ¢ÃÂÃÂ Needs Improvement" count={contentBad.length} borderColor="rgba(110,43,255,0.3)" headerBg={C.accent} titleColor="#fff"><div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 10 }}>{contentBad.map((p, i) => <ProblemCard key={i} {...p} />)}</div></Fold></div>}
    ...
    {trustGood.length > 0 && <div className="reveal" style={{ marginBottom: 12 }}><Fold title="Trust & Conversion ÃÂ¢ÃÂÃÂ Working Well" count={trustGood.length} borderColor={C.cardBorder} headerBg={C.card}><div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 10 }}>{trustGood.map((g, i) => <WorkingItem key={i} title={g.title} content={g.content} />)}</div></Fold></div>}
    ...
    {trustBad.length > 0 && <div className="reveal" style={{ marginBottom: 20 }}><Fold title="Trust & Conversion ÃÂ¢ÃÂÃÂ Needs Improvement" count={trustBad.length} borderColor="rgba(110,43,255,0.3)" headerBg={C.accent} titleColor="#fff"><div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 10 }}>{trustBad.map((p, i) => <ProblemCard key={i} {...p} />)}</div></Fold></div>}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {allBad.map((item, i) => {const pr=PRIO[item.priority]||PRIO.important;return(<div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
            <span style={{ color: pr.color, fontSize: 10, marginTop: 4 }}>ÃÂ¢ÃÂÃÂ</span>
            <div style={{ flex: 1 }}>
...
async function generateCoveragePDF(data) {
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ SCORE CARD ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  content.push({
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ KEYWORDS TABLE ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ CONTENT WORKING WELL (detailed, like Core Audit PDF) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ CONTENT NEEDS IMPROVEMENT (with SERP preview + full suggestions) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ TRUST WORKING WELL (with details) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ TRUST NEEDS IMPROVEMENT ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ FINAL RECOMMENDATIONS ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ BUILD DOCUMENT ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  const docDefinition = {
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ GENERATE + DOWNLOAD + UPLOAD ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  pdfDocGen.getBlob(async (pdfBlob) => {
    try {
      ...
      if (upMemberId && pdfBlob) {
        ...
        console.log("[CC] PDF not uploaded ÃÂ¢ÃÂÃÂ no member ID");
...
/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ PLACEHOLDER ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
const CoveragePlaceholder = () => <div style={{ minHeight: "calc(100vh - 180px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 40 }}>
  ...
  <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6, textAlign: "center", maxWidth: 340, marginBottom: 24 }}>I'll analyze your page and show you what's missing ÃÂ¢ÃÂÃÂ so you know exactly what to fix to rank higher and convert more visitors.</div>
...
function ContentCoverage({ onHome, memberName: mn }) {
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ REAL AUDIT PIPELINE ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  const runAudit = async (url, keywords, usedExtracted = false) => {
    ...
    try {
      ...
      // Step 2: GPT ÃÂ¢ÃÂÃÂ extract page context + keywords
      setStepNum(2);
      let gptData = null;
      try {
      ...
      // Step 4: DFS ÃÂ¢ÃÂÃÂ volume/KD + SERP (PAA, related, autocomplete)
      setStepNum(4);
      let dfsData = null;
      try {
      ...
      /* v6.2: Use serp_positions from DFS ÃÂ¢ÃÂÃÂ real Google positions for all keywords */
      const serpPositions = dfsData?.serp_positions || {};

      // Build keyword metrics ÃÂ¢ÃÂÃÂ extracted keywords table (fallback to ranked data for volume/KD)
      ...
      if (serpAutocomplete.length === 0 && serpRelated.length === 0 && serpPaa.length === 0) {
        console.log("[CC] DFS SERP empty ÃÂ¢ÃÂÃÂ falling back to GPT serp_suggestions");
        try {
      ...
      // Word count (basic ÃÂ¢ÃÂÃÂ count words in visible text)
      ...
      setTimeout(() => {
        sTyp(false);
        bot(<div><div style={{fontWeight:600,marginBottom:8}}>{"Done! I found " + totalIssues + " areas that need attention."}</div><div style={{color:C.muted,fontSize:12,marginBottom:8}}>Each card has a clear fix ÃÂ¢ÃÂÃÂ tap to see what to do. {isMobile ? "Switch to the Report tab" : "Check the report on the right"} for the full breakdown.</div><div style={{fontWeight:600}}>Ask me anything ÃÂ¢ÃÂÃÂ I can explain any issue or help you fix it.</div></div>);
        setStep("done");
      }, 1000);
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ CHAT ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  ...
  const send = () => {
      ...
      (async () => {
        try {
          ...
          bot(<div>
            ...
            <div style={{ marginBottom: 10, padding: "10px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}` }}>{kw.map((k, i) => <div key={i} style={{ fontSize: 12, fontWeight: 400, color: C.dark, padding: "2px 0" }}>ÃÂ¢ÃÂÃÂ¢ {k}</div>)}</div>
            <div style={{ color: C.muted, fontSize: 12, marginBottom: 6 }}>I'll use these keywords to deeply analyze your page ÃÂ¢ÃÂÃÂ checking how well your content covers them, where they appear in your structure, and what's missing.</div>
    ...
    if (step === "own_keywords") {
      // User typed their own keywords ÃÂ¢ÃÂÃÂ clean separators, send to GPT for cleaning
      const cleanedInput = text.replace(/[ÃÂ¢ÃÂÃÂ¢ÃÂÃÂ·\-ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ|\/\\]/g, ",").replace(/,{2,}/g, ",");
      sTyp(true);
      (async () => {
        try {
          const res = await fetch(COVERAGE_GPT, {
          ...
          bot(<div>
            ...
            <div style={{ marginBottom: 10, padding: "10px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}` }}>{cleaned.map((k, i) => <div key={i} style={{ fontSize: 12, fontWeight: 400, color: C.dark, padding: "2px 0" }}>ÃÂ¢ÃÂÃÂ¢ {k}</div>)}</div>
          ...
          bot(<div>
            ...
            <div style={{ marginBottom: 10, padding: "10px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}` }}>{fallback.map((k, i) => <div key={i} style={{ fontSize: 12, fontWeight: 400, color: C.dark, padding: "2px 0" }}>ÃÂ¢ÃÂÃÂ¢ {k}</div>)}</div>
    ...
    if (step === "adjust_keywords") {
      // User wants to adjust ÃÂ¢ÃÂÃÂ send feedback to GPT again
      sTyp(true);
      (async () => {
        try {
          const res = await fetch(COVERAGE_GPT, {
    ...
    if (step === "done" || showR) {
      // Check if user typed a new URL ÃÂ¢ÃÂÃÂ offer re-audit with credit check
      const v = valUrl(text);
      if (v.ok) {
        (async () => {
          try {
  ...
  const chatMessages = <React.Fragment>
    ...
    {step === "keywords" && extractedKw && <div style={{ display: "flex", gap: 8, marginTop: 4, flexWrap: "wrap" }}><Btn text="Use These Keywords" onClick={() => { add("u", "Use these keywords"); setUserKw(extractedKw); setStep("running"); runAudit(pageUrl, extractedKw, true); }} /><Btn text="Write My Own" onClick={() => { setStep("own_keywords"); bot(<div>
            <div style={{ fontWeight: 600, marginBottom: 6 }}>Type 1ÃÂ¢ÃÂÃÂ3 keyword phrases you want to rank for.</div>
            <div style={{ color: C.muted, fontSize: 12, marginBottom: 10 }}>Each phrase should be 2ÃÂ¢ÃÂÃÂ3 words ÃÂ¢ÃÂÃÂ the way real people search in Google. Separate with commas.</div>
