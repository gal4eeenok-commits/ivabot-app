...
const Tip = ({ text, children }) => { const [s, setS] = useState(false); const ref = useRef(null); const posRef = useRef({ top: 0, left: 0, above: false }); return (<span ref={ref} style={{ position: "relative", display: "inline-flex", alignItems: "center" }} onMouseEnter={() => { if (ref.current) { const rect = ref.current.getBoundingClientRect(); const above = rect.top > 180; const alignRight = rect.left > window.innerWidth - 280; posRef.current = { top: above ? rect.top - 8 : rect.bottom + 8, left: alignRight ? Math.max(8, rect.right - 260) : Math.max(8, rect.left), above }; } setS(true); }} onMouseLeave={() => setS(false)}>{children}{s && (() => { const p = posRef.current; return <span style={{ position: "fixed", top: p.above ? "auto" : p.top, bottom: p.above ? (window.innerHeight - p.top) + "px" : "auto", left: p.left, background: C.surface, color: C.dark, padding: "10px 14px", borderRadius: 10, fontSize: 11, lineHeight: 1.5, width: 260, maxWidth: "85vw", zIndex: 9999, fontWeight: 400, boxShadow: "0 4px 24px rgba(0,0,0,0.14)", border: `1px solid ${C.border}`, pointerEvents: "none", whiteSpace: "normal", wordBreak: "break-word", textAlign: "left" }}>{text}</span>; })()}</span>); };
