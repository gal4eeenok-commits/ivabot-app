...
function buildResults(d) {
  ...
  if (d.linksStatus === "good") g.push({ title: "Links & Social Profiles", content: (<><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><StatCard number={d.links.internal} label="Internal Links" desc="Connect your pages so Google can discover and rank them." borderColor={NB} /><StatCard number={d.links.external} label="External Links" desc={d.links.external === 0 ? "No external links found. Linking to trusted sources builds credibility and trust with Google." : "Linking to trusted sources builds credibility."} borderColor={d.links.external === 0 ? "rgba(110,43,255,0.2)" : NB} /><StatCard number={d.backlinksCount != null ? d.backlinksCount.toLocaleString() : "?"} label="Backlinks" desc={d.backlinksCount === 0 ? "No backlinks — critical for rankings." : d.backlinksCount != null ? `From ${d.referringDomains?.toLocaleString() || "—"} referring domains.` : "Backlinks are links from other websites to yours — one of Google's top 3 ranking factors."} borderColor={(d.backlinksCount === 0 || d.backlinksCount == null) ? "rgba(110,43,255,0.3)" : NB} /></div>{(d.backlinksCount == null || d.backlinksCount === 0 || d.backlinksCount < 10) && <div style={{ marginTop: 8, padding: "10px 14px", borderRadius: 8, background: "rgba(110,43,255,0.04)", border: "1px solid rgba(110,43,255,0.15)" }}><div style={{ fontSize: 12, fontWeight: 600, color: C.dark, marginBottom: 2 }}>{d.backlinksCount === 0 ? "No backlinks detected" : d.backlinksCount != null && d.backlinksCount < 10 ? "Low backlink count" : "Backlinks matter"}</div><div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5 }}>{d.backlinksCount === 0 ? "Without backlinks, it's very hard to rank on page one — even with perfect on-page SEO." : d.backlinksCount != null && d.backlinksCount < 10 ? "With fewer than 10, you're likely losing rankings to competitors with stronger link profiles." : "Backlinks are one of Google's top 3 ranking factors. Each quality link from another site is a 'vote of confidence' that helps you rank higher."} Check the PR & Backlinks section below for specific opportunities.</div></div>}{d.links.social.length > 0 ? (<div><div style={{ fontSize: 10, fontWeight: 600, color: C.muted, marginBottom: 6 }}>Social Profiles</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{d.links.social.map((s, i) => <SocialBadge key={i} name={typeof s === "string" ? s : s.name} url={typeof s === "string" ? null : s.url} />)}</div></div>) : (<div style={{ fontSize: 12, color: C.muted, padding: "8px 0" }}>No social profiles linked on this page. Consider adding social links in your header or footer to build brand trust.</div>)}<BotNote inline text="A healthy mix of internal links, external references, and social profiles tells Google your site is well-connected and trustworthy." /></>) }); else b.push(d.linksEval);
...
const RankingsTable = ({ rows, emptyMsg }) => (
  ...
  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, tableLayout: "fixed" }}>
    <colgroup><col style={{ width: "auto" }} /><col style={{ width: 56 }} /><col style={{ width: 80 }} /><col style={{ width: 56 }} /></colgroup>
    <thead><tr style={{ borderBottom: `1px solid ${C.border}` }}>
      ...
      <th style={{ textAlign: "right", padding: "8px 4px", color: C.muted, fontWeight: 500, fontSize: 11.5, position: "relative" }}>Volume<span style={{ position: "absolute", top: 4, marginLeft: 1 }}><QM text="Monthly search volume — how many times per month people search this keyword in Google." /></span></th>
      <th style={{ textAlign: "right", padding: "8px 0", color: C.muted, fontWeight: 500, fontSize: 11.5, position: "relative" }}>KD<span style={{ position: "absolute", top: 4, marginLeft: 1 }}><QM text="Keyword difficulty (0–100) — how hard it is to rank in the top 10. Under 30 = easy, 30–60 = moderate, 60+ = hard." /></span></th>
    </tr></thead>
...
const ReportV6 = ({ data, onNewAudit, onHome }) => { const { good, bad } = buildResults(data); return (<div style={{ maxWidth: 580, margin: "0 auto", padding: "20px 16px 16px" }}>
  ...
  <div className="reveal" style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
    <Fold title="Top Competitors in Google" borderColor={C.cardBorder} headerBg={C.card}><BotNote inline text={`These are the top organic Google results for "${data.keywords?.[0] || "your topic"}". They currently outrank your page for this search query.`} /><div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>{data.competitors.map((c, i) => (<div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}>
      ...
      {(c.title || c.tactics) && <div style={{ fontSize: 12.5, color: "#1a0dab", fontWeight: 500, paddingLeft: 32, marginBottom: 2 }}>{c.title || c.tactics}</div>}
      {c.description && <div style={{ fontSize: 11.5, color: C.muted, paddingLeft: 32, marginBottom: 4, lineHeight: 1.4 }}>{c.description.length > 120 ? c.description.slice(0, 117) + "..." : c.description}</div>}
