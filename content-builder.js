...
const STOP_HL="(?:to|in|for|from|during|of|the|a|and|on|with|by)";
const hlF=(text,kd)=>{if(!text||!kd?.length)return text;const sorted=[...kd].filter(x=>x.text&&x.text.split(/\s+/).length>=2).sort((a,b)=>(b.text?.length||0)-(a.text?.length||0));if(!sorted.length)return text;let p=[{t:text,h:false,f:null}];sorted.forEach(({text:kw,freq})=>{const words=kw.split(/\s+/).filter(w=>w.length>0);if(words.length<2)return;const escaped=words.map(w=>w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'));const re=new RegExp(`(${escaped.join(`(?:\\s+${STOP_HL})?\\s+`)})`,'gi');const np=[];p.forEach(pt=>{if(pt.h){np.push(pt);return;}const parts=pt.t.split(re);for(let i=0;i<parts.length;i++){const s=parts[i];if(!s)continue;if(re.test(s)){np.push({t:s,h:true,f:freq});re.lastIndex=0;}else{np.push({t:s,h:false,f:null});}}});p=np;});return<span>{p.map((x,i)=>{if(!x.h)return<span key={i}>{x.t}</span>;const fc=FC[x.f]||FC.MV;return<span key={i} style={{background:fc.bg,color:fc.color,padding:"1px 4px",borderRadius:3}}>{x.t}</span>;})}</span>;};
...
const STOP_WORDS="(?:to|in|for|from|during|of|the|a|and|on|with|by)";
const buildSoftRegex=(kw)=>{const words=kw.split(/\s+/).filter(w=>w.length>0);if(words.length<2)return null;const escaped=words.map(w=>w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'));return new RegExp(`\\b(${escaped.join(`(?:\\s+${STOP_WORDS})?\\s+`)})\\b`,'gi');};
