...
const InfoBlock = ({ label, value, borderColor }) => {
  const renderLine = (line, i) => {
    ...
    if (hMatch) {
      ...
      return (<div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 500, color: isBroken ? C.red : C.dark, padding: "2px 0" }}>
        <span style={{ fontSize: 9, fontWeight: 600, color: isBroken ? C.red : C.muted, background: isBroken ? "rgba(239,68,68,0.08)" : C.bg, padding: "2px 5px", borderRadius: 3, minWidth: 22, textAlign: "center", flexShrink: 0 }}>{lv}</span>
        <span>{text}</span>
      </div>);
    }
