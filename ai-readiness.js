...
function AIReadinessTool({ onHome, memberName: mn }) {
  ...
  const runAIReadiness = async (url) => {
    ...
    try {
      ...
      bot(<div><div style={{ fontWeight: 600, marginBottom: 6 }}>{"Done. Your AI readiness score is " + aiReadiness.score + " out of 100."}</div><div style={{ color: C.muted, fontSize: 12 }}>{nBad + " signal" + (nBad !== 1 ? "s" : "") + " to improve. "}{isMobile ? "Switch to the Report tab" : "See the report on the right"} and ask me about any of them.</div></div>);
    } catch (e) {
