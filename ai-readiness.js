    ...
    try {
      ...
      const _pAio = (async () => {
        try {
          ...
          var _tracked=[]; try{ var _hraw=localStorage.getItem("iva_air_handoff"); if(_hraw){ var _hobj=JSON.parse(_hraw); if(_hobj&&_hobj.at&&(Date.now()-_hobj.at)<600000&&_hobj.prompts&&(!_hobj.dom||String(_hobj.dom).toLowerCase()===String(_h).toLowerCase())){ Object.keys(_hobj.prompts).forEach(function(_pk2){ var _rec=_hobj.prompts[_pk2]; var _q2=String((_rec&&_rec.prompt)||_pk2||"").trim(); if(_q2&&_tracked.indexOf(_q2)===-1)_tracked.push(_q2); }); } } }catch(_te){ console.warn("[AIR] tracked prompts could not be read", _te); }
