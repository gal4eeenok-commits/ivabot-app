/* IvaBot seo-tools v81 â PDF redesign, NicheBadge/LowBadge in RankingsTable */
(function() {
...
const WorkingItem = ({ title, content }) => { const [o, setO] = useState(false); return (<div style={{ borderRadius: 10, border: `1px solid ${C.cardBorder}`, overflow: "hidden", background: C.surface }}><button onClick={() => setO(!o)} style={{ width: "100%", padding: "11px 14px", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontFamily: "'DM Sans',sans-serif" }}><span style={{ color: "#9B7AE6", flexShrink: 0, fontSize: 13, fontWeight: 600 }}>â</span><span style={{ fontSize: 13, fontWeight: 600, color: C.dark, flex: 1, textAlign: "left" }}>{title}</span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={C.muted} strokeWidth="2" strokeLinecap="round" style={{ transform: o ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.2s", flexShrink: 0 }}><polyline points="6 9 12 15 18 9" /></svg></button>{o && <div style={{ padding: "0 14px 14px", borderTop: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 10 }}>{content}</div></div>}</div>); };

/* Unified block â Current value. borderColor determines green/red/grey */
const InfoBlock = ({ label, value, borderColor }) => {
  ...
  const renderLine = (line, i) => {
    ...
    if (hMatch) {
      ...
      const isBroken = text.includes("â ");
...
/* Unified explanation â sireneviy bg, used for both "why good" and "why bad" */
...
/* Stat card â white bg, colored border */
...
/* Problem Card â purple accent border, bot explains inside */
...
const ProblemCard = ({ title, why, currentLabel, current, suggestions, sugLabel, showCopy = true, links, serpSnippet, soft, priority }) => { const [o, setO] = useState(false); const pr=PRIO[priority]||PRIO.important; return (<div style={{ borderRadius: 12, border: soft?"1px solid rgba(110,43,255,0.12)":"1px solid rgba(110,43,255,0.25)", overflow: "hidden", background: C.surface }}><button onClick={() => setO(!o)} style={{ width: "100%", padding: "13px 16px", background: C.surface, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontFamily: "'DM Sans',sans-serif" }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: pr.color, flexShrink: 0 }} /><span style={{ fontSize: 13, fontWeight: 600, color: C.dark, flex: 1, textAlign: "left" }}>{title}</span><span style={{fontSize:9,fontWeight:600,color:pr.color,background:pr.bg,padding:"3px 8px",borderRadius:6,textTransform:"uppercase",letterSpacing:"0.5px",flexShrink:0}}>{pr.label}</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.muted} strokeWidth="2" strokeLinecap="round" style={{ transform: o ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.2s", flexShrink: 0 }}><polyline points="6 9 12 15 18 9" /></svg></button>{o && (<div style={{ padding: "0 16px 16px", borderTop: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
  ...
  {links?.length > 0 && <div><div style={{ fontSize: 10, fontWeight: 600, color: C.muted, marginBottom: 6 }}>Learn more</div>{links.map((l, i) => (<a key={i} href={l.url} target="_blank" rel="noopener noreferrer" style={{ display: "block", fontSize: 12, color: C.accent, marginBottom: 4, textDecoration: "none" }}>{l.label} â</a>))}</div>}
</div></div>)}</div>); };
...
/* âââ CHAT BUBBLES (Builder style) âââ */
...
/* âââ Loading & Placeholder panels âââ */
...
/* Get user info â Supabase Auth (with fallback to window globals set by Dashboard) */
...
/* Fetch credits from Supabase â lookup by user_id first, then member_id */
...
function buildReportData(parsed, gpt, dfs) {
  ...
  const keywordMetrics = gptKeywords.map(k => {
    ...
    /* Try exact match first, then partial (contains), then word overlap â search ALL ranked keywords */
  ...
  /* For "How Your Page Ranks" table â show top 7 by volume (DFS already sorts by volume desc) */
  ...
  return {
    ...
    titleEval: parsed.title_status !== "good" ? { why: parsed.title_missing ? "No meta title found â Google can't generate a proper search snippet." : parsed.title_has_repeated_brand ? `Your title repeats the brand name ("${parsed.title}"). This wastes valuable characters and looks unprofessional in search results. Use the space to describe what the page offers.` : parsed.title_too_short ? "Too short â users and Google need more context to understand what this page offers." : "Title is too long â Google will truncate it in search results.", sugLabel: "Suggested Titles", suggestions: (Array.isArray(gpt?.suggested_titles) && gpt.suggested_titles.length > 0) ? gpt.suggested_titles : (gpt?.suggested_title ? (Array.isArray(gpt.suggested_title) ? gpt.suggested_title : [gpt.suggested_title]) : (gpt?.keywords?.length > 0 ? gpt.keywords.map(k => k.charAt(0).toUpperCase() + k.slice(1) + " â " + (gpt?.page_context?.owner || parsed.hostname)) : ["Add a descriptive title with your primary keyword"])), showCopy: true, links: [{ label: "Google Search Console", url: "https://search.google.com/search-console" }] } : null,
    descStatus: parsed.desc_status === "good" ? "good" : "bad",
    descEval: parsed.desc_status !== "good" ? { why: parsed.desc_missing ? "No meta description found â search engines can't generate a proper snippet." : parsed.desc_too_short ? "Description is too short â aim for 120-160 characters." : "Description is too long â Google may truncate it.", sugLabel: "Suggested Descriptions", suggestions: (Array.isArray(gpt?.suggested_descriptions) && gpt.suggested_descriptions.length > 0) ? gpt.suggested_descriptions : (gpt?.suggested_description ? (Array.isArray(gpt.suggested_description) ? gpt.suggested_description : [gpt.suggested_description]) : ["Write a 120-160 character description including your primary keyword"]), showCopy: true } : null,
    ...
    speedStatus: "bad", speedEval: { title: "Page Speed Check", why: "Speed is a Core Web Vital â slow pages lose visitors and rank lower.", suggestions: ["Compress images to WebP", "Remove unused JS/CSS", "Enable lazy loading"], showCopy: false, links: [{ label: "Google PageSpeed Insights", url: "https://pagespeed.web.dev/" }] },
...
function buildResults(d) {
  ...
  if (d.headingsStatus === "good") { const h1 = d.headings.filter(h => h.level === "H1"), h2 = d.headings.filter(h => h.level === "H2"), h3 = d.headings.filter(h => h.level === "H3");
    ...
    const HL = ({ tags, lv }) => (<div style={{ display: "flex", flexDirection: "column", gap: 3 }}>{tags.map((h, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 500, color: h.broken ? C.accent : C.dark }}><span style={{ fontSize: 9, fontWeight: 600, color: h.broken ? C.accent : hColors[lv].color, background: h.broken ? "rgba(110,43,255,0.08)" : hColors[lv].bg, padding: "2px 5px", borderRadius: 3, minWidth: 22, textAlign: "center" }}>{lv}</span>{h.text}{h.broken && <span style={{ fontSize: 10, color: C.accent, fontWeight: 600 }}>â  unrendered</span>}</div>))}</div>);
    g.push({ title: "Heading Structure", content: (<>
      <InfoBlock label={`H1 â ${h1.length} found`} value={<HL tags={h1} lv="H1" />} borderColor={NB} />
      <InfoBlock label={`H2 â ${h2.length} found`} value={<HL tags={h2} lv="H2" />} borderColor={NB} />
      <InfoBlock label={`H3 â ${h3.length} found`} value={<HL tags={h3} lv="H3" />} borderColor={NB} />
      <BotNote inline text="Think of headings as a table of contents. H1 is your main topic, H2s are chapters, H3s are subsections. Google uses this hierarchy to understand your page. Yours looks clean â no duplicates detected." />
  ...
  if (d.ux.cta.found) g.push({ title: "Call to Action", content: (<><InfoBlock label="Current CTA" value={`"${d.ux.cta.text}"`} borderColor={NB} /><BotNote inline text="A clear call-to-action guides visitors to the next step â buy, sign up, learn more. Pages with visible CTAs convert better." /></>) }); else b.push({ title: "No CTA Found", priority: "important", why: "A call-to-action (CTA) is a button or link that guides visitors to take action â like 'Buy now', 'Sign up', or 'Contact us'. Without one, visitors may leave without converting.", suggestions: ["Add a prominent CTA above the fold"], showCopy: false });
  if (d.ux.mobile) g.push({ title: "Mobile Optimization", content: (<><InfoBlock label="Status" value="Your page looks great on phones and tablets." borderColor={NB} /><BotNote inline text="Over 60% of searches happen on mobile. Google ranks mobile-friendly pages higher â and yours is ready." /></>) }); else b.push({ title: "Not Mobile-Friendly", priority: "critical", why: "Your page doesn't have a viewport meta tag, which means it won't display properly on phones and tablets. Google uses mobile-first indexing, so this directly hurts your rankings.", suggestions: ["Add viewport meta tag to your page head"], showCopy: false });
  ...
  if (!d.ux.noVideo) g.push({ title: "Video Content", content: (<><InfoBlock label="Status" value="Video detected on your page." borderColor={NB} /><BotNote inline text="Pages with video keep visitors engaged longer. Google notices this â it's a positive ranking signal." /></>) }); else b.push({...d.videoEval, priority: "nice"});
  if (d.speedStatus === "good") g.push({ title: "Page Speed", content: (<><InfoBlock label="Status" value="Your page loads fast." borderColor={NB} /><BotNote inline text="Speed is a Core Web Vital â Google uses it directly for rankings. Fast pages also have lower bounce rates and happier visitors." /></>) }); else b.push({...d.speedEval, priority: "important"});
  if (d.robotsStatus === "good") g.push({ title: "robots.txt", content: (<><InfoBlock label="Status" value={<>Properly configured. <a href={(()=>{ try { return new URL(d.url).origin+"/robots.txt"; } catch(e) { return "#"; } })()} target="_blank" rel="noopener noreferrer" style={{ color: C.accent, textDecoration: "none", fontSize: 12 }}>View your robots.txt â</a></>} borderColor={NB} /><BotNote inline text="This file tells search engines which pages to crawl and which to skip. Yours is set up correctly." /></>) }); else b.push({ title: "robots.txt Not Found", priority: "nice", why: "robots.txt is a small file at the root of your site that tells search engines which pages to crawl and which to skip. Without it, Google may waste time crawling unnecessary pages or miss important ones. Most website platforms can generate this automatically.", suggestions: ["Create robots.txt at your domain root"], showCopy: false, links: [{ label: "How to create robots.txt", url: "https://developers.google.com/search/docs/crawling-indexing/robots/create-robots-txt" }] });
  if (d.sitemapStatus === "good") g.push({ title: "Sitemap", content: (<><InfoBlock label="Status" value={<>XML sitemap is accessible. <a href={(()=>{ try { return new URL(d.url).origin+"/sitemap.xml"; } catch(e) { return "#"; } })()} target="_blank" rel="noopener noreferrer" style={{ color: C.accent, textDecoration: "none", fontSize: 12 }}>View your sitemap â</a></>} borderColor={NB} /><BotNote inline text="A sitemap is like a roadmap for Google â it helps discover and index all your pages faster." /></>) }); else b.push({ title: "Sitemap Not Found", priority: "nice", why: "An XML sitemap is a list of all your pages that helps Google discover and index them faster. Without one, new or deep pages may not appear in search results for weeks. Most CMS platforms generate sitemaps automatically â check your settings.", suggestions: ["Generate and submit XML sitemap"], showCopy: false, links: [{ label: "Google Sitemap Guide", url: "https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap" }] });
...
const PaymentToast = () => {
  ...
  return (<div style={{ position: "fixed", top: 24, left: 0, right: 0, zIndex: 300, display: "flex", justifyContent: "center", pointerEvents: "none", fontFamily: "'DM Sans',sans-serif" }}>
    <div style={{ width: "95%", maxWidth: 440, animation: "fadeIn 0.3s ease", pointerEvents: "auto", background: C.surface, border: "1px solid rgba(110,43,255,0.15)", borderRadius: 12, padding: "16px 20px", display: "flex", alignItems: "center", gap: 14, boxShadow: "0 8px 24px rgba(110,43,255,0.08)" }}>
      ...
      <button onClick={() => setShow(false)} style={{ background: "none", border: "none", color: C.muted, fontSize: 16, cursor: "pointer", padding: 4, lineHeight: 1, fontFamily: "'DM Sans',sans-serif" }}>â</button>
    </div>
  </div>);
};
...
const BuyM = ({ onClose, memberId }) => {
  ...
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}><span style={{ fontSize: 20, fontWeight: 700, color: C.dark }}>Buy Credits</span><button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: C.muted, fontSize: 18 }}>â</button></div>
...
/* Bot voice â with logo between sections, plain inside cards */
...
const RankingsTable = ({ rows, emptyMsg }) => (
  ...
  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
    <thead><tr style={{ borderBottom: `1px solid ${C.border}` }}>
      ...
      <th style={{ textAlign: "right", padding: "8px 4px", color: C.muted, fontWeight: 500, fontSize: 11.5, width: 70, whiteSpace: "nowrap" }}>Vol. <QM text="Monthly search volume â how many times per month people search this keyword in Google." /></th>
      <th style={{ textAlign: "right", padding: "8px 0", color: C.muted, fontWeight: 500, fontSize: 11.5, width: 50, whiteSpace: "nowrap" }}>KD <QM text="Keyword difficulty (0â100) â how hard it is to rank in the top 10. Under 30 = easy, 30â60 = moderate, 60+ = hard." /></th>
    </tr></thead>
...
async function generatePDF(data) {
  ...
  /* ââ Color palette (hex for pdfmake) ââ */
  ...
  /* Badge helper â table with colored bg */
  ...
  /* ââ Score Circle (canvas â dataURL) ââ */
  ...
  /* ââ Card builder (lavender bg for Needs Improvement) ââ */
  const lavCard = (item) => {
  ...
  /* ââ White card builder (for Final Recommendations) ââ */
  ...
  /* ââââââââââââââââââââââââââââââââââââââââââââââ
     NEEDS IMPROVEMENT â lavender cards
     ââââââââââââââââââââââââââââââââââââââââââââââ */
  const pB = [];
  ...
  /* ââââââââââââââââââââââââââââââââââââââââââââââ
     FINAL RECOMMENDATIONS â white cards
     ââââââââââââââââââââââââââââââââââââââââââââââ */
  ...
  pdfDocGen.getBlob(async (pdfBlob) => {
    try {
      ...
      if (upMemberId && pdfBlob) {
        ...
        console.log("[IvaBot] PDF not uploaded â no member ID (user not logged in)");
...
const ReportV6 = ({ data, onNewAudit, onHome }) => { const { good, bad } = buildResults(data); return (<div style={{ maxWidth: 580, margin: "0 auto", padding: "20px 16px 16px" }}>
  <BotNote text="Here's your full SEO audit. I'll walk you through each part â what's working, what needs fixing, and exactly how to fix it." />
  <div className="reveal" style={{ display: "flex", gap: 16, marginBottom: 18, padding: 16, borderRadius: 14, background: C.card, border: `1px solid ${C.cardBorder}` }}><ScoreRing score={data.score} /><div style={{ flex: 1 }}><div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 2 }}><span style={{ fontSize: 11, fontWeight: 500, color: C.muted }}>SEO Score</span><QM text="Your overall score based on title, description, headings, speed, mobile, links, robots.txt, and sitemap. Higher is better â aim for 80+." /></div><div style={{ fontSize: 14, fontWeight: 700, color: C.dark, marginBottom: 4, wordBreak: "break-all" }}>{data.url}</div><div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.4 }}>{data.score >= 80 ? "Your page has a strong foundation. Let's fine-tune the details below." : "There's room for improvement â I'll show you exactly what to fix."}</div></div></div>
  ...
  {data.rankedKeywords?.length > 0 && <div className="reveal reveal-delay-2" style={{ marginBottom: 16, padding: 16, borderRadius: 12, background: C.card, border: `1px solid ${C.cardBorder}` }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 13, fontWeight: 700, color: C.dark }}>How your page ranks in Google</span><QM text="Real positions from the Google search index â these are the actual search queries your page currently ranks for." /></div>
    ...
    <div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5, marginBottom: 10 }}>Keywords your page actually ranks for in Google right now â with real positions from the search index.</div>
    <RankingsTable rows={data.rankedKeywords} />
    <BotNote inline text="Positions 1â3 mean strong visibility. 4â10 is page one but below the fold. 11+ means page two or deeper â most users never scroll there." />
  ...
  <div className="reveal reveal-delay-2" style={{ marginBottom: 20, padding: 16, borderRadius: 12, background: C.card, border: `1px solid ${C.cardBorder}` }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 13, fontWeight: 700, color: C.dark }}>What your page is built for</span><QM text="Based on your title, H1âH3 headings, and meta description â these are the search queries your content is currently optimized for." /></div>
    ...
    <div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5, marginBottom: 10 }}>Based on your title, headings (H1âH3), and meta description â these are the search queries your content is currently optimized for.</div>
  ...
  <BotNote text={good.length > 0 ? `Good news â ${good.length} things are already working well on your page.` : "Let's look at what needs attention."} />
  ...
  <BotNote text={bad.length > 0 ? `I found ${bad.length} areas that need attention. Each card has a clear fix â tap to see what to do.` : "Everything looks great! No major issues found."} />
  ...
  <div className="reveal" style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
    <Fold title="Top Competitors in Google" borderColor={C.cardBorder} headerBg={C.card}><BotNote inline text={`These are the top organic Google results for "${data.keywords?.[0] || "your topic"}". They currently outrank your page for this search query.`} /><div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>{data.competitors.map((c, i) => (<div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}>
      ...
      {c.url && <a href={c.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: C.accent, textDecoration: "none", paddingLeft: 32, display: "block" }}>{c.url.length > 55 ? c.url.slice(0, 52) + "..." : c.url} â</a>}
      ...
      <div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {item.links?.[0] && <a href={item.links[0].url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: C.accent, textDecoration: "none" }}>{item.links[0].label} â</a>}
          </div>
        </div>
      </div>
    );})}
    <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${(data.backlinksCount == null || data.backlinksCount < 10) ? "rgba(110,43,255,0.25)" : C.cardBorder}` }}><div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}><span style={{ color: C.accent, fontSize: 10, marginTop: 4 }}>â</span><div><div style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 2 }}>{data.backlinksCount != null && data.backlinksCount >= 10 ? "Keep building backlinks" : "Build quality backlinks â this is critical"}</div><div style={{ fontSize: 11.5, color: C.muted }}>{data.backlinksCount != null && data.backlinksCount < 10 ? `You currently have ${data.backlinksCount === 0 ? "no" : "only " + data.backlinksCount} backlink${data.backlinksCount !== 1 ? "s" : ""}. Backlinks are one of Google's top 3 ranking factors â without them, even perfect on-page SEO won't be enough. ` : ""}Reach out to industry blogs, directories, and publications. Offer guest posts, case studies, or resource mentions. Check the PR & Backlinks section above for specific opportunities.</div></div></div></div>
    <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}><span style={{ color: C.accent, fontSize: 10, marginTop: 4 }}>â</span><div><div style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 2 }}>Create useful content</div><div style={{ fontSize: 11.5, color: C.muted }}>Content that solves real problems for your users is the foundation of lasting SEO success.</div></div></div></div>
    <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}><span style={{ color: C.accent, fontSize: 10, marginTop: 4 }}>â</span><div><div style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 2 }}>Monitor with Google tools</div><div style={{ fontSize: 11.5, color: C.muted }}>Use <a href="https://search.google.com/search-console" target="_blank" rel="noopener noreferrer" style={{ color: C.accent, textDecoration: "none" }}>Search Console</a> and <a href="https://pagespeed.web.dev/" target="_blank" rel="noopener noreferrer" style={{ color: C.accent, textDecoration: "none" }}>PageSpeed Insights</a> to track improvements.</div></div></div></div>
...
/* âââ MAIN â NEW LAYOUT (Builder-style) âââ */
function IvaBotV6() {
  ...
  /* Reveal on scroll for report â staggered to prevent white screen */
  useEffect(() => {
    if (!showR) return;
  ...
  /* Sticky fix â CSS override instead of JS DOM manipulation to prevent layout jumping */
    ...
    try {
      if (USE_MOCK) {
        ...
        if (dfsResult.status === "fulfilled" && dfsResult.value) {
          ...
          console.log("[IvaBot] DFS OK â ranked:", dfsSeo.ranked_keywords.length, "serp:", dfsSeo.serp_competitors.length);
        } else {
      ...
      if (!USE_MOCK) {
        ...
        try {
          ...
          const runRes = await fetch(SUPABASE_URL + "/rest/v1/rpc/insert_core_run", {
            method: "POST",
            headers: { "Content-Type": "application/json", "apikey": SUPABASE_KEY, "Authorization": "Bearer " + getAuthJWT() },
      ...
      const summary = d.score >= 80
        ? `Your site is in good shape â ${good.length} things are working well${bad.length > 0 ? ` and ${bad.length} small improvements can push you even higher` : ""}.`
        : d.score >= 50
          ? `You have strong foundations, but ${bad.length} areas need attention to reach your full potential.`
          : `There's a lot of room for growth â ${bad.length} key areas need work, but every fix will make a real difference.`;
      setTyping(true);
      setTimeout(() => {
        setTyping(false);
        addMsg("bot", <div>
          ...
          <div style={{ color: C.muted, fontSize: 12 }}>{isMobile ? "Switch to the Report tab to see details." : "Check the report on the right for details."} Come back and re-audit once you've made changes â I'll track your progress.</div>
        </div>);
  ...
  const sendChat = async (text) => {
    ...
    if (USE_MOCK) {
      const mockAnswers = [
        ...
        "Your title is only 5 characters â Google needs more context to understand your page. Aim for 30-60 characters with your main keyword.",
  ...
  /* âââ Chat Messages â stable, no animation re-trigger âââ */
  ...
  /* Right panel content â always shows something, never blank */
  ...
  return (
    <div className="iva-root" style={{ fontFamily: "'DM Sans',sans-serif", background: "#f8f7f9", display: "flex", flexDirection: "column", padding: "8px 12px", minHeight: "100vh" }}>
      ...
      {view === "select" ? (
        ...
        /* âââ CORE AUDIT â Builder-style layout âââ */
        <div style={{ fontFamily: "'DM Sans',sans-serif", flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          {!isMobile && <div style={{ display: "flex", padding: "0 24px 24px", maxWidth: 1224, margin: "0 auto", width: "100%", alignItems: "flex-start", gap: 12 }}>
            ...
            {/* Right panel â scrolls with page */}
      ...
      <div style={{ padding: "6px 16px 4px", background: "transparent", textAlign: "center", flexShrink: 0 }}><span style={{ fontSize: 10, color: C.muted }}>Powered by IvaBot Â· AI SEO Assistant</span></div>
      </div>
    </div>
  );
}
