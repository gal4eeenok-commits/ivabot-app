...
function parseSEO(rawHtml, pageUrl) {
  ...
  // Score
  let sc = 0;
  const ts = r.title_missing ? "missing" : r.title_too_short ? "too_short" : r.title_too_long ? "too_long" : "good";
...
function buildResults(d) {
  ...
  const g = [], b = [];
  if (d.titleStatus === "good") g.push({ title: "Meta Title", content: (<><InfoBlock label="Current Title" value={`"${d.title}"`} borderColor={NB} /><BotNote inline text="Your title is the right length (30–60 characters) and clearly describes the page. This is the #1 on-page signal Google uses to understand your content." /></>) }); else b.push({ ...d.titleEval, title: d.titleEval?.title || "Meta Title Too Short" });
  if (d.descStatus === "good") g.push({ title: "Meta Description", content: (<><InfoBlock label="Current Description" value={`"${d.desc}"`} borderColor={NB} /><BotNote inline text="Your description is within 120–160 characters — the sweet spot. This is what users see in search results, so a good one means more clicks." /></>) }); else b.push({ ...d.descEval, title: d.descEval?.title || "Description Needs Work" });
  ...
  if (d.linksStatus === "good") g.push({ title: "Links & Social Profiles", content: (<><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><StatCard number={d.links.internal} label="Internal Links" desc="Connect your pages so Google can discover and rank them." borderColor={NB} /><StatCard number={d.links.external} label="External Links" desc={d.links.external === 0 ? "No external links found. Linking to trusted sources builds credibility and trust with Google." : "Linking to trusted sources builds credibility."} borderColor={d.links.external === 0 ? "rgba(110,43,255,0.2)" : NB} />{d.backlinksCount != null && <StatCard number={d.backlinksCount.toLocaleString()} label="Backlinks" desc={d.backlinksCount === 0 ? "No backlinks found. This is critical — backlinks are one of Google's top 3 ranking factors." : `From ${d.referringDomains?.toLocaleString() || "—"} referring domains. Backlinks are one of Google's top 3 ranking factors.`} borderColor={d.backlinksCount === 0 ? "rgba(110,43,255,0.3)" : NB} />}</div>{d.backlinksCount === 0 && <div style={{ marginTop: 8, padding: "10px 14px", borderRadius: 8, background: "rgba(110,43,255,0.04)", border: "1px solid rgba(110,43,255,0.15)" }}><div style={{ fontSize: 12, fontWeight: 600, color: C.dark, marginBottom: 2 }}>No backlinks detected</div><div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5 }}>Backlinks are one of the most important ranking signals. Without them, it's very difficult to rank on page one — even with perfect on-page SEO. Start by submitting your site to relevant directories, writing guest posts, and reaching out to industry publications.</div></div>}{d.links.social.length > 0 ? (<div><div style={{ fontSize: 10, fontWeight: 600, color: C.muted, marginBottom: 6 }}>Social Profiles</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{d.links.social.map((s, i) => <SocialBadge key={i} name={typeof s === "string" ? s : s.name} url={typeof s === "string" ? null : s.url} />)}</div></div>) : (<div style={{ fontSize: 12, color: C.muted, padding: "8px 0" }}>No social profiles linked on this page. If this is a public-facing page (portfolio, services, blog), consider adding social links in your header or footer to build brand trust.</div>)}<BotNote inline text="A healthy mix of internal links, external references, and social profiles tells Google your site is well-connected and trustworthy." /></>) }); else b.push(d.linksEval);
...
const ReportV6 = ({ data, onNewAudit, onHome }) => { const { good, bad } = buildResults(data); return (<div style={{ maxWidth: 580, margin: "0 auto", padding: "20px 16px 16px" }}>
  ...
  {data.rankedKeywords?.length > 0 && <div className="reveal reveal-delay-2" style={{ marginBottom: 16, padding: 16, borderRadius: 12, background: C.card, border: `1px solid ${C.cardBorder}` }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 13, fontWeight: 700, color: C.dark }}>Your real Google rankings</span><QM text="Real positions from the Google index — these are the actual search queries your page currently ranks for." /></div>
  ...
  <div className="reveal" style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
    <Fold title="Competitor SEO Tactics" borderColor={C.cardBorder} headerBg={C.card}><BotNote inline text="Understanding what your competitors do well is one of the fastest ways to improve. These are the top 3 Google results for your topic — study their strengths and apply what works." /><div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>{data.competitors.map((c, i) => (<div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}><NumBadge n={i + 1} /><span style={{ fontSize: 13, fontWeight: 600, color: C.dark }}>{c.name}</span></div><div style={{ fontSize: 11.5, color: C.muted, paddingLeft: 28 }}>{c.tactics}</div></div>))}</div></Fold>
    <Fold title="PR & Backlink Opportunities" borderColor={C.cardBorder} headerBg={C.card}><BotNote inline text="Backlinks are one of the top 3 ranking factors. Every quality link from another website is a 'vote of confidence' that tells Google your content is worth ranking higher. Even 2–3 strong backlinks can make a real difference." /><div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>{data.backlinks.map((b, i) => (<div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}><NumBadge n={i + 1} /><span style={{ fontSize: 13, fontWeight: 600, color: C.dark }}>{b.name}</span></div><div style={{ fontSize: 11.5, color: C.muted, paddingLeft: 28 }}>{b.desc}</div></div>))}</div></Fold>
  </div>
  ...
  <div className="reveal" style={{ marginBottom: 8, padding: 20, borderRadius: 14, background: C.card, border: `1px solid ${C.cardBorder}` }}><div style={{ fontSize: 15, fontWeight: 700, color: C.dark, marginBottom: 12 }}>Final Recommendations</div><div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
    ...
    <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}><span style={{ color: C.accent, fontSize: 10, marginTop: 4 }}>●</span><div><div style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 2 }}>Build quality backlinks</div><div style={{ fontSize: 11.5, color: C.muted }}>Even 2–3 links from reputable sources can significantly boost your authority. Check the backlink opportunities above.</div></div></div></div>
...
function IvaBotV6() {
  ...
  return (
    <div className="iva-root" style={{ fontFamily: "'DM Sans',sans-serif", background: "#f8f7f9", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden", padding: "12px 12px" }}>
      ...
      {view === "select" ? (
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          {showR && (
            <div className="iva-actions" style={{ flexShrink: 0, alignItems: "center", padding: "6px 24px 0" }}>
              ...
              <button onClick={() => alert("PDF export")} style={{ height: 40, padding: "0 20px", borderRadius: 10, background: C.surface, border: `1px solid ${C.borderMid}`, color: C.dark, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = C.accentLight} onMouseLeave={e => e.currentTarget.style.background = C.surface}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" /></svg>Export PDF</button>
      ...
      <div style={{ padding: "12px 16px 8px", background: "transparent", textAlign: "center", flexShrink: 0 }}><span style={{ fontSize: 10, color: C.muted }}>Powered by IvaBot · AI SEO Assistant</span></div>
      </div>
    </div>
  );
}
