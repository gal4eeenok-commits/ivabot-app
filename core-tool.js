  ...
  const runAudit = async (url) => {
    ...
    try {
      if (USE_MOCK) {
        ...
        else { locale = detectLocale(url, parsed.html_lang, parsed.hreflang, { text: (((parsed.title||"")+" "+(parsed.body_text||"")).slice(0,8000)) }); try{ await ivaKeepPageMarket(url, locale); }catch(_km){ console.warn("[IvaBot] the market could not be kept for this page", _km); } }
