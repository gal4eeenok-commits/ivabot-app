...
const ReportV6 = ({ data, onNewAudit, onHome }) => { const { good, bad } = buildResults(data);
  ...
  {data.rankedKeywords?.length > 0 && <div className="reveal reveal-delay-2" style={{ marginBottom: 16, borderRadius: 12, border: `1px solid ${C.cardBorder}`, background: C.surface }}>
    ...
    <div style={{ padding: "2px 16px 12px" }}>
      {data.totalRanked != null && <div style={{ padding: "13px 0", borderBottom: "1px solid rgba(21,20,21,0.06)", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}><div style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ fontSize: 13, fontWeight: 600, color: C.dark }}>Keywords your page ranks for</span><QM text="Total search queries this page ranks for in Google, from the search index." /></div><div style={{ textAlign: "right", flexShrink: 0 }}><span style={{ fontSize: 18, fontWeight: 700, color: C.dark }}>{fmtVol(data.totalRanked)}</span><div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>in Google index</div></div></div>}
  ...
  <div className="reveal" style={{ marginBottom: 16, borderRadius: 12, border: `1px solid ${C.cardBorder}`, background: C.surface }}>
    ...
    <div style={{ padding: "2px 16px 12px" }}>
      ...
      <div style={{ marginTop: 12 }}><Fold title="PR & outreach opportunities" count={data.backlinks.length} borderColor={C.cardBorder} headerBg={C.card}><BotNote inline text="Every quality link from another website is a vote of confidence for Google. Even 2–3 strong backlinks can make a real difference." /><div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 8 }}>{data.backlinks.map((b, i) => (<div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.cardBorder}` }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}><NumBadge n={i + 1} /><span style={{ fontSize: 13, fontWeight: 600, color: C.dark }}>{b.name}</span></div><div style={{ fontSize: 11.5, color: C.muted, paddingLeft: 28 }}>{b.desc}</div></div>))}</div></Fold></div>
    </div>
  </div>
