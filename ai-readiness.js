/* IvaBot AI Readiness (standalone) v4.19 - v4.19 - the answers of a run are tied to it three times, not once. The dashboard writes its Google AI answers a few seconds after the run row appears, so a single pass at the moment the run is recorded missed them; the repeat passes at four and twenty seconds pick them up, and they can only touch answers of this site from this run that carry no run of their own. v4.18 - v4.18 - the charge call names the page it is for, so the wallet works the price out from the prompts tracked for that site instead of trusting the number this file sends. The number is still sent and is still used when the wallet cannot price the run itself. v4.17 - the brand-mention request is made once per run instead of twice, and the helpers for the handoff, the refund and the run tagging are visible to every caller. v4.16 - the answers are tied to the run that paid for them. prompt_answers carries a run_id column now, and since the dashboard writes those rows before this file creates the run, the tool fills the column itself: every answer of this site written since the moment the dashboard half started, and still without a run, gets this run id. The start time comes from the handoff, and half an hour back is used when there is none. Nothing is tagged in sandbox. v4.15 - a run that stops after the credit was taken now gives it back. charge_credit returns the id of its ledger row, the tool keeps it, and if the run ends in the error branch the tool asks the credit-refund function to return that exact charge. The refund reads the account from the token, accepts one charge once and refuses a charge that belongs to someone else or is older than an hour, so the path cannot be used to make credits. The message to the person says the credit came back. v4.14 - the sandbox reaches this tool, and the handoff belongs to one page. Until now the flag set on the dashboard was known only to the dashboard, so a run started with iva_sandbox=1 still charged a credit, still paid DataForSEO and still wrote a run row, a snapshot and a report. The tool reads the same key from localStorage, marks every DataForSEO call as sandbox, takes no credit and writes nothing in that mode. The handoff from the dashboard is now matched on the page as well as the site, so a run on an inner page stops taking the numbers of the home page, and the key is removed once the snapshot is written, so the next run starts from what it measured itself. v4.13 - an inner page pays the base two credits. The price counted the prompts of the whole site whatever page was run, because prompts are stored per site, so a run on one article of a twenty-prompt site cost six credits and re-checked prompts the article never shows. The prompt count now sets the price only when the run is on the home page, where the prompts are tracked and displayed; every other page pays two for its own audit. v4.12 - this file reads the market of the page, not only of the site. The lookup asked dash_state for the domain, so a page given its own market in the dashboard was still checked in the market of its site here, while the dashboard half of the same run used the page market; the two halves of one run could measure different countries. The page address is set before the lookup and tried first, exactly as core-tool.js does it. v4.11 - a run on a tracked site repeats none of the dashboard work. Until now this file asked DataForSEO for the AI Overview of three questions and then for nine engine answers, three prompts across ChatGPT, Perplexity and Gemini, all of which the dashboard had just paid for; the cache could not help because its key is the whole request body and the dashboard sends ten questions at a time while this file sent three. The handoff already carries every prompt with its result for today, so the rows the report needs are built from it and neither call is made. Both calls still happen for a first run, when no dashboard result exists. v4.10 - a run for a site that already tracks prompts costs one request less. v4.9 stopped the tool inventing questions but still fetched the ranked keywords that fed them; the handoff is now read first, and when it carries prompts the ranked_keywords call is skipped entirely. The old path is untouched for a first run, when there is nothing to track yet. v4.9 - the tool stops inventing questions for a site that already tracks prompts. Its own check built three questions out of ranked keywords on every run, so a site with twenty-two tracked prompts still paid for three questions nobody follows, and the report showed those instead of the tracked ones. It now reads the prompts the dashboard hands over in localStorage under iva_air_handoff and uses the first three; the old path from ranked keywords and air-gpt stays exactly as it was for a first run, when there is nothing to track yet. v4.8 - the snapshot carries what the dashboard actually checked. v4.7 took only the citation count from the address, so the tracked prompts and the overview count came from this file, which knows nothing about them: prompts_checked held the three keyword questions this tool builds for itself and ai_overview_count was a zero. The dashboard now leaves its whole result in localStorage under iva_air_handoff, and the snapshot prefers it for the prompts, the overview count and the citations, falling back to the address and then to this file when the handoff is missing, stale or from another site. The version printed on load said v4.4 for four versions and now matches the file. v4.7 - the snapshot carries the citation count. The dashboard half counts how many tracked prompts brought an answer to the site and used to write its own ai_readiness snapshot for that one number, which left two snapshots and two run rows per launch. It now hands the number over in the address as iva_cites and writes nothing itself, so this file writes the only run row and the only snapshot, with p_ai_citations_count filled from that parameter. Opened directly from /app, with no parameter, the field stays null as before. Publish together with dashboard-tool.js v544. v4.6 - the citation check on the top three keywords asked ChatGPT and Perplexity only, and then wrote gemini as a flat false into the prompt record, so the dashboard displayed a definite not-cited for an engine that had never been asked. Google AI is now asked as well, with the same model the dashboard uses, gemini-2.5-flash, and the record carries the real answer. This is three more llm_responses calls per run, one per keyword. v4.5 - the check used to go ahead whenever the charge call failed for any reason other than an explicit refusal, so a network error or a timeout produced a free run that still cost money at DataForSEO. The run now starts only on a confirmed ok from the wallet, and says which of the two happened: not enough credits, or the charge could not be taken. v4.4 - a saved report opened from the dashboard says which day it belongs to, the way the Core report now does. The page carried no date, so an old report and a fresh one looked identical and the only way to tell them apart was the hint on the link. The date comes from the report row in the same query, and when it cannot be read the previous wording is used unchanged. v4.3 - a saved report opens again. Its payload keeps the "content" of each passed signal as a React element, which JSON strips down to a bare object, so React threw error #31 and rendered nothing even though the row itself had been fetched. The payload is rebuilt into real elements before it reaches the report. v4.2 — the detected locale is now also written into the AI Readiness snapshot, which previously stored location_code and language_code as null even when the check itself ran against the right country. v4.1 — the AI Overview block now detects the page locale through the detectLocale helper that already lived in this file and was never called, so a Romanian or Dutch site is measured against its own SERP instead of the American one. v4.0 — brand mentions now respect the brand the user set by hand in the dashboard (localStorage iva_dash_brand_<userId>, keyed by host); a run from /app can no longer overwrite it. The page-title guess was removed from the brand chain, because a title like "Suave - Shop Now, Pay Over Time" resolved to the brand "Suave" and counted an unrelated same-name brand (159,512 instead of 488). Chain is now: brand set by hand, then og:site_name, then the domain's second-level label. Nothing else changed. v3.9 — PDF (generateAIReadinessPDF) now mirrors the real on-screen report exactly: it adds only the Where to mention this page (distribution tips) block, placed after the AI citations & authority note (which stands in for the live TrustTable + Prompt visibility blocks that live in the dashboard) and before the On-page AI signals sections. No Coverage-clone sections (Page Context, keywords table, Content & Structure, Trust & Conversion) are added — those belong to the dead CoverageReport, not the AI Readiness report. v3.8 — cloned from content-coverage.js shell; AI Readiness report for only, free preview, whitelist-gated. v3.1 change: removed the page backlinks block and the Google reviews / local-rating block (these now belong to Core Audit); the open-web mentions row is renamed to Brand mentions across the web. v3.2 change: distribution tips are now generated per page and vertical via the air-gpt distribution_tips step, with fallback to the static page-type tips. v3.3 change: tips load in the background after the report is shown, so the report never blocks; the tips block shows page-type tips instantly and quietly upgrades to the tailored ones when they arrive. v3.4 change: trimmed the completion message to score, signals, and a short invite to ask. v3.5 change: each completed analysis is recorded to Run history via insert_air_run (flow_type=ai_readiness); credit charge deferred until AI metrics are live and the whitelist is lifted. */
(function() {
const{useState,useRef,useEffect,useCallback}=React;
console.log("[IvaBot] ai-readiness.js (standalone) v4.19 loaded");
...
/* v4.10: the sandbox reaches the tool. The flag is set on the dashboard and kept in localStorage under iva_sbx,
   and the tool runs on the same origin, so it reads the same key. In this mode the DataForSEO sandbox answers,
   no credit is taken and nothing is written to the live tables. */
const IVA_AIR_SBX=(function(){try{if(/[?&]iva_sandbox=1/.test(location.search))return true;if(/[?&]iva_sandbox=0/.test(location.search))return false;return localStorage.getItem("iva_sbx")==="1";}catch(e){return false;}})();
if(IVA_AIR_SBX){console.warn("[AIR] SANDBOX mode is on: DataForSEO answers are test data, no credit is charged and no row is written. Open the dashboard with ?iva_sandbox=0 to turn it off.");
  if(typeof window!=="undefined"&&!window.__ivaSbxFetch){window.__ivaSbxFetch=1;const _ivaLiveFetch=window.fetch.bind(window);
    window.fetch=function(u,o){try{if(String(u).indexOf("/functions/v1/dataforseo-proxy")>=0&&o&&typeof o.body==="string"){const b=JSON.parse(o.body);if(b&&!b.sandbox){b.sandbox=true;o=Object.assign({},o,{body:JSON.stringify(b)});}}}catch(e){console.warn("[AIR] a request could not be marked as sandbox:",e);}return _ivaLiveFetch(u,o);};
    console.warn("[AIR] every DataForSEO call from this page now goes to the sandbox");}}
...
async function recordCoverageRun(memberId,url){if(IVA_AIR_SBX){console.warn("[CC] sandbox run, no run row is written");return null;}try{const isUUID=/^[0-9a-f]{8}-/.test(memberId);const runBody=isUUID?{p_user_id:memberId,p_source_url:url||null}:{p_member_id:memberId,p_source_url:url||null};const res=await fetch(`${SUPABASE_URL}/rest/v1/rpc/insert_coverage_run`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+(await ivaAuthToken()),"apikey":SUPABASE_KEY},body:JSON.stringify(runBody)});let runId=null;try{const d=await res.json();runId=d&&d.run_id?d.run_id:null;}catch(_){}console.log("[CC] run recorded:",runId);return runId;}catch(e){console.error("[CC] run record error:",e);return null;}}
/* v4.17: the handoff, the refund and the run tagging are read from three places in this file, but
   they were declared inside recordAirSnapshot, so every call from outside it threw a reference error:
   the answers were never tied to their run, and a failed run never returned its credit because the
   refund call died the same way. The same four helpers live here at the top of the file, where every
   caller can see them. The copies inside recordAirSnapshot are left untouched and simply shadow these
   within that one function, so nothing that already worked changes. */
function ivaAirNorm(u){try{var x=new URL(String(u).indexOf("http")===0?u:"https://"+u);return (x.hostname.replace(/^www\./,"")+x.pathname.replace(/\/+$/,"")).toLowerCase();}catch(e){return String(u||"").toLowerCase();}}
function ivaAirHandoff(dm,pg){try{var raw=localStorage.getItem("iva_air_handoff");if(!raw)return null;var o=JSON.parse(raw);if(!o||!o.at)return null;if((Date.now()-o.at)>600000){console.log("[AIR] handoff is stale, ignoring it");return null;}if(dm&&o.dom&&String(o.dom).toLowerCase()!==String(dm).toLowerCase()){console.log("[AIR] handoff belongs to another site, ignoring it");return null;}if(pg&&o.page&&ivaAirNorm(o.page)!==ivaAirNorm(pg)){console.log("[AIR] handoff belongs to another page,",o.page,"so it is ignored here");return null;}return o;}catch(e){console.warn("[AIR] handoff could not be read",e);return null;}}
async function ivaAirRefund(ledgerId,reason){if(!ledgerId)return null;try{const r=await fetch(`${SUPABASE_URL}/functions/v1/credit-refund`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+(await ivaAuthToken()),"apikey":SUPABASE_KEY},body:JSON.stringify({ledger_id:ledgerId,reason:reason||"the run produced nothing"})});const d=await r.json();if(d&&d.ok)console.log("[AIR] credit returned:",d.refunded,"balance",d.balance);else console.warn("[AIR] the credit was not returned:",d&&d.reason);return d;}catch(e){console.warn("[AIR] refund call failed",e);return null;}}
async function ivaAirTagAnswers(userId,domain,runId,since){if(IVA_AIR_SBX){console.warn("[AIR] sandbox run, the answers are not tagged");return 0;}if(!userId||!domain||!runId)return 0;try{const from=since||new Date(Date.now()-1800000).toISOString();const q=`${SUPABASE_URL}/rest/v1/prompt_answers?user_id=eq.${encodeURIComponent(userId)}&domain=eq.${encodeURIComponent(domain)}&run_id=is.null&checked_at=gte.${encodeURIComponent(from)}`;const r=await fetch(q,{method:"PATCH",headers:{"Content-Type":"application/json","Authorization":"Bearer "+(await ivaAuthToken()),"apikey":SUPABASE_KEY,"Prefer":"return=representation"},body:JSON.stringify({run_id:runId})});if(!r.ok){const t=await r.text();console.warn("[AIR] the answers could not be tied to the run:",r.status,t.slice(0,200));return 0;}const rows=await r.json();console.log("[AIR] answers tied to this run:",(rows&&rows.length)||0,"since",from);return (rows&&rows.length)||0;}catch(e){console.warn("[AIR] tying the answers to the run failed",e);return 0;}}
async function recordAirRun(memberId,url){if(IVA_AIR_SBX){console.warn("[AIR] sandbox run, no run row is written");return null;}try{const isUUID=/^[0-9a-f]{8}-/.test(memberId);const runBody=isUUID?{p_user_id:memberId,p_source_url:url||null}:{p_member_id:memberId,p_source_url:url||null};const res=await fetch(`${SUPABASE_URL}/rest/v1/rpc/insert_air_run`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+(await ivaAuthToken()),"apikey":SUPABASE_KEY},body:JSON.stringify(runBody)});let runId=null;try{const d=await res.json();runId=d&&d.run_id?d.run_id:null;}catch(_){}console.log("[AIR] run recorded:",runId);return runId;}catch(e){console.error("[AIR] run record error:",e);return null;}}
async function recordAirSnapshot(a){if(IVA_AIR_SBX){console.warn("[AIR] sandbox run, no snapshot is written");return null;}try{var __adm=(typeof window!=="undefined"&&window.__ivaAdmin&&window.__ivaAdmin.userId)?window.__ivaAdmin:null;if(__adm){a=Object.assign({},a,{memberId:__adm.userId});}var ai=a.aiReadiness||{};var good=ai.aiGood||[];var bad=ai.aiBad||[];var passed=good.length;var total=ai.total||(good.length+bad.length);var score=(typeof ai.score==="number")?Math.round(ai.score):null;var coverage={score:score,signals_passed:passed,signals_total:total,page_type:ai.pageType||null,good:good.map(function(g){return g&&g.title;}).filter(Boolean),bad:bad.map(function(b){return b&&b.title;}).filter(Boolean),aio:(a.aioItems&&a.aioItems.length?a.aioItems:null),brand:(a.brand||null)};function ivaAirNorm(u){try{var x=new URL(String(u).indexOf("http")===0?u:"https://"+u);return (x.hostname.replace(/^www\./,"")+x.pathname.replace(/\/+$/,"")).toLowerCase();}catch(e){return String(u||"").toLowerCase();}}
function ivaAirHandoff(dm,pg){try{var raw=localStorage.getItem("iva_air_handoff");if(!raw)return null;var o=JSON.parse(raw);if(!o||!o.at)return null;if((Date.now()-o.at)>600000){console.log("[AIR] handoff is stale, ignoring it");return null;}if(dm&&o.dom&&String(o.dom).toLowerCase()!==String(dm).toLowerCase()){console.log("[AIR] handoff belongs to another site, ignoring it");return null;}if(pg&&o.page&&ivaAirNorm(o.page)!==ivaAirNorm(pg)){console.log("[AIR] handoff belongs to another page,",o.page,"so it is ignored here");return null;}return o;}catch(e){console.warn("[AIR] handoff could not be read",e);return null;}}
async function ivaAirRefund(ledgerId,reason){if(!ledgerId)return null;try{const r=await fetch(`${SUPABASE_URL}/functions/v1/credit-refund`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+(await ivaAuthToken()),"apikey":SUPABASE_KEY},body:JSON.stringify({ledger_id:ledgerId,reason:reason||"the run produced nothing"})});const d=await r.json();if(d&&d.ok)console.log("[AIR] credit returned:",d.refunded,"balance",d.balance);else console.warn("[AIR] the credit was not returned:",d&&d.reason);return d;}catch(e){console.warn("[AIR] refund call failed",e);return null;}}
async function ivaAirTagAnswers(userId,domain,runId,since){if(IVA_AIR_SBX){console.warn("[AIR] sandbox run, the answers are not tagged");return 0;}if(!userId||!domain||!runId)return 0;try{const from=since||new Date(Date.now()-1800000).toISOString();const q=`${SUPABASE_URL}/rest/v1/prompt_answers?user_id=eq.${encodeURIComponent(userId)}&domain=eq.${encodeURIComponent(domain)}&run_id=is.null&checked_at=gte.${encodeURIComponent(from)}`;const r=await fetch(q,{method:"PATCH",headers:{"Content-Type":"application/json","Authorization":"Bearer "+(await ivaAuthToken()),"apikey":SUPABASE_KEY,"Prefer":"return=representation"},body:JSON.stringify({run_id:runId})});if(!r.ok){const t=await r.text();console.warn("[AIR] the answers could not be tied to the run:",r.status,t.slice(0,200));return 0;}const rows=await r.json();console.log("[AIR] answers tied to this run:",(rows&&rows.length)||0,"since",from);return (rows&&rows.length)||0;}catch(e){console.warn("[AIR] tying the answers to the run failed",e);return 0;}}
function ivaAirHandoffClear(){try{localStorage.removeItem("iva_air_handoff");console.log("[AIR] handoff used and cleared");}catch(e){console.warn("[AIR] handoff could not be cleared",e);}}var _ho=ivaAirHandoff(a.domain,a.url);if(_ho)console.log("[AIR] handoff from the dashboard: prompts",(_ho.prompts?Object.keys(_ho.prompts).length:0),"overviews",_ho.aio,"citations",_ho.cites);var isUUID=/^[0-9a-f]{8}-/.test(a.memberId||"");var body={p_domain:a.domain||null,p_user_id:isUUID?a.memberId:null,p_member_id:isUUID?null:(a.memberId||null),p_flow_type:"ai_readiness",p_run_id:a.runId||null,p_audit_score:score,p_coverage:coverage,p_ai_mentions_count:(a.ai_mentions_count!=null)?a.ai_mentions_count:null,p_ai_citations_count:(function(){try{if(_ho&&_ho.cites!=null)return _ho.cites;if(a.ai_citations_count!=null)return a.ai_citations_count;var _q=new URLSearchParams(window.location.search).get("iva_cites");if(_q==null||_q==="")return null;var _n=parseInt(_q,10);return isNaN(_n)?null:_n;}catch(e){console.warn("[AIR] citation count could not be read",e);return null;}})(),p_ai_search_volume:(a.ai_search_volume!=null)?a.ai_search_volume:null,p_ai_overview_count:((_ho&&_ho.aio!=null)?_ho.aio:((a.ai_overview_count!=null)?a.ai_overview_count:null)),p_prompts_checked:((_ho&&_ho.prompts)?_ho.prompts:(a.prompts_checked||null)),p_subject:"self",p_location_code:(a.location_code!=null)?a.location_code:null,p_language_code:(a.language_code!=null)?a.language_code:null};if(__adm){body.p_member_id=null;console.log("[AIR][ADMIN] payload for "+__adm.userId+":",JSON.stringify(body));var ares=await fetch(`${SUPABASE_URL}/functions/v1/admin-snapshot-write`,{method:"POST",headers:{"Content-Type":"application/json","apikey":SUPABASE_KEY},body:JSON.stringify({admin_secret:(__adm.secret||""),snapshot:body})});var adata=null;try{adata=await ares.json();}catch(e){}console.log("[AIR][ADMIN] result",ares.status,JSON.stringify(adata));return adata;}var res=await fetch(`${SUPABASE_URL}/rest/v1/rpc/insert_snapshot`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+(await ivaAuthToken()),"apikey":SUPABASE_KEY},body:JSON.stringify(body)});if(res.ok){var dd=await res.json();console.log("[AIR] snapshot recorded:",JSON.stringify(dd));ivaAirHandoffClear();return dd;}var t="";try{t=await res.text();}catch(e){}console.error("[AIR] snapshot HTTP",res.status,t.slice(0,200));return null;}catch(e){console.error("[AIR] snapshot error:",e);return null;}}
...
async function ivaManualLocale(dom){
  try{
    ...
    for(var _i=0;_i<_keys.length;_i++){ var r=v[_keys[_i]]; if(r&&r.loc){ console.log("[AIR] market taken from the dashboard for", _keys[_i], r.loc, r.lang||"en"); return {location_code:r.loc,language_code:(r.lang||"en")}; } }
...
function AIReadinessTool({ onHome, memberName: mn }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (_rep) {
        ...
        (async () => {
          try {
            const _rr = await fetch(`${SUPABASE_URL}/rest/v1/reports?select=payload,created_at&id=eq.${encodeURIComponent(_rep)}&flow_type=eq.ai_readiness`, {
            ...
            if (_row && _row.payload) {
              ...
              const _rd = (function () { try { var d = new Date(_row.created_at); if (isNaN(d.getTime())) return ""; return d.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }); } catch (e) { return ""; } })(); console.log("[AIR] saved report of", _rd || "an unknown date"); add("b", _rd ? ("Here's your saved AI Readiness report from " + _rd + ". Ask me anything about it.") : "Here's your saved AI Readiness report. Ask me anything about it.");
            } else {
  ...
  const runAIReadiness = async (url) => {
    /* Charge up front: 2 credits for a run of up to five tracked prompts, two more for every ten after that. The count comes from dash_state, which the dashboard keeps. The check starts only on a confirmed charge: a wallet that answers ok:false, an HTTP error and a network failure all stop the run, because a run that goes ahead unpaid still costs money at DataForSEO. */
    let _paid = false, _why = "unavailable", _chargeId = null;
    try {
      const _mid = getMemberId();
      const _isU = _mid && /^[0-9a-f]{8}-/.test(_mid);
      let _cost = 2;
      let _isHomeRun = false;
      try { const _p0 = new URL(/^https?:/i.test(url) ? url : ("https://" + url)).pathname.replace(/\/$/, ""); _isHomeRun = (_p0 === "" || _p0 === "/"); } catch (e) { _isHomeRun = false; }
      if (!_isHomeRun) console.log("[AIR] inner page: prompts belong to the site and are checked from its home page, so this run costs the base 2");
      if (_isU && _isHomeRun) {
        try {
          let _host = "";
      ...
      if (IVA_AIR_SBX) { _paid = true; console.warn("[AIR] sandbox run, no credit was charged"); } else {
      const _chBody = _isU ? { p_user_id: _mid, p_action: "ai_readiness", p_cost: _cost, p_domain: url, p_page: url } : { p_member_id: _mid, p_action: "ai_readiness", p_cost: _cost, p_domain: url, p_page: url };
    ...
    if (!_paid) {
      console.warn("[AIR] the check did not start because the credit was not taken:", _why);
      try { sPLoad(null); } catch(e) {}
      try { sTyp(false); } catch(e) {}
      try { alert(_why === "insufficient" ? "Not enough credits to run AI Readiness. Buy more credits to continue." : "The credit could not be taken just now, so the check did not start. Try again in a moment."); } catch(e) {}
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (gptRes.ok) { const gpt = await gptRes.json(); pageType = (gpt && gpt.page_type) || "other"; if (gpt && gpt.page_context && gpt.page_context.owner) { const _o = String(gpt.page_context.owner).trim(); if (_o.length >= 2 && _o.length <= 40 && /[a-zA-Z\u0400-\u04FF]/.test(_o) && _o.indexOf(".") === -1) _summaryBrand = _o; } if (gpt && gpt.keywords && gpt.keywords.length) { _gptKws = gpt.keywords.filter(function (x) { return typeof x === "string" && x.trim(); }).slice(0, 3); } }
      } catch (e) {}
      ...
      const _pMentions = (async () => {
        try {
          ...
          var _brand = ""; var _brandAuto = true;
          try { var _uid = (window.__userId || window.__memberId || (typeof getMemberId === "function" ? getMemberId() : null)); if (_uid) { var _bm = JSON.parse(localStorage.getItem("iva_dash_brand_" + _uid) || "{}"); if (_bm && typeof _bm[_host] === "string" && _bm[_host].trim()) { _brand = _bm[_host].trim(); _brandAuto = false; } } } catch (e) {}
          ...
          /* v4.17: the dashboard half already asked DataForSEO for the brand mentions of this site and
             leaves the number in the handoff. Asking again cost ten cents a run, more than every prompt
             answer of that run put together, for a figure already in hand. The request is made only when
             there is no handoff, which is the case when this tool is opened on its own. */
          var _mr = null, _hoM = null;
          try { _hoM = ivaAirHandoff(_host, null); } catch (_hm) { console.warn("[AIR] handoff could not be read for the brand mentions", _hm); }
          if (_hoM && _hoM.mentions != null) {
            _mr = { mentions: _hoM.mentions, ai_search_volume: (_hoM.aisv != null ? _hoM.aisv : 0) };
            console.log("[AIR] brand mentions taken from the dashboard run:", _hoM.mentions, "- no second request");
          } else {
            _mr = await ivaFetchRetry(DFS_PROXY, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + (await ivaAuthToken()) }, body: JSON.stringify({ mode: "llm_mentions", brand: _brand, target: _host, platform: "google" }) }).then(function (r) { return (r && r.ok) ? r.json() : null; });
          }
          if (_mr) {
      ...
      const _airLoc = await (async function () {
        try {
        try{ var _pkA=String(url||"").toLowerCase().replace("https://","").replace("http://",""); if(_pkA.charAt(_pkA.length-1)==="/")_pkA=_pkA.slice(0,-1); window.__ivaPageKey=_pkA; }catch(_pkB){ window.__ivaPageKey=""; }
      ...
      console.log("[AIR] locale:", _airLoc.location_code, _airLoc.language_code);
      try { window.__ivaAirLoc = _airLoc; } catch (_e) {}
      const _pAio = (async () => {
        try {
          ...
          var _tracked=[]; var _hoMap=null; try{ var _hraw=localStorage.getItem("iva_air_handoff"); if(_hraw){ var _hobj=JSON.parse(_hraw); if(_hobj&&_hobj.at&&(Date.now()-_hobj.at)<600000&&_hobj.prompts&&(!_hobj.dom||String(_hobj.dom).toLowerCase()===String(_h).toLowerCase())){ _hoMap=_hobj.prompts; Object.keys(_hobj.prompts).forEach(function(_pk2){ var _rec=_hobj.prompts[_pk2]; var _q2=String((_rec&&_rec.prompt)||_pk2||"").trim(); if(_q2&&_tracked.indexOf(_q2)===-1)_tracked.push(_q2); }); } } }catch(_te){ console.warn("[AIR] tracked prompts could not be read", _te); }
          if(_tracked.length)console.log("[AIR] using the prompts you track:", _tracked.length, "so no keywords are fetched and no questions are invented");
          var _rk = _tracked.length ? null : await ivaFetchRetry(DFS_PROXY, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + (await ivaAuthToken()) }, body: JSON.stringify({ mode: "ranked_only", domain: _h, location_code: _airLoc.location_code, language_code: _airLoc.language_code }) }).then(function (r) { return (r && r.ok) ? r.json() : null; });
          var _pk = _nrm(url);
...
/* seeds: real search demand for this page or domain; questions are generated from them */
          ...
          var _seeds = _tracked.length ? _tracked.slice(0, 5) : _kws.slice(0, 5);
          ...
          var _qs = _tracked.length ? _tracked.slice(0, 3) : [];
          ...
          var _fromDash = false; var _ao = null;
          if (_hoMap) { try { var _td = new Date().toISOString().slice(0, 10); var _rows = [];
            Object.keys(_hoMap).forEach(function (_k3) { var _r3 = _hoMap[_k3] || {}; var _q3 = String(_r3.prompt || _k3 || "").trim(); if (!_q3) return;
              var _mn = ((_r3.runs || {})[_td]) || {}; var _ct = ((_r3.runsC || {})[_td]) || {};
              _rows.push({ q: _q3, triggers: !!_r3.aio_triggers, cited: !!_ct.ai_overview, chat_gpt: !!_ct.chat_gpt, perplexity: !!_ct.perplexity, gemini: !!_ct.gemini, mentioned: !!_mn.ai_overview }); });
            if (_rows.length) { _ao = { ai_overview: _rows }; _fromDash = true; _kws = _rows.slice(0, 3).map(function (x) { return x.q; });
              console.log("[AIR] Google AI and engine results taken from the dashboard run:", _rows.length, "prompts, no extra requests"); }
          } catch (_hd) { console.warn("[AIR] the dashboard results could not be reused", _hd); } }
          if (!_ao) _ao = await ivaFetchRetry(DFS_PROXY, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + (await ivaAuthToken()) }, body: JSON.stringify({ mode: "ai_overview", keywords: _kws, target: _h, location_code: _airLoc.location_code, language_code: _airLoc.language_code }) }).then(function (r) { return (r && r.ok) ? r.json() : null; });
          if (_ao && Array.isArray(_ao.ai_overview) && _ao.ai_overview.length) {
            ...
            /* For the top-3 keywords, also check citation in each of the three engines via llm_responses. Gemini was missing here until v4.6 and, worse, was written into the record as a definite not-cited, so the dashboard showed a negative for an engine nobody had asked. The models match the ones the dashboard uses, so the two sides answer the same question the same way. */
            try {
              if (_fromDash) throw { skip: true };
              var _tok = await ivaAuthToken();
              var _top3 = _ao.ai_overview.slice(0, 3);
              var _llmJobs = [];
              _top3.forEach(function (row) {
                [["chat_gpt", "gpt-4o-mini"], ["perplexity", "sonar"], ["gemini", "gemini-2.5-flash"]].forEach(function (_em) {
                  var eng = _em[0];
              ...
              _ao.ai_overview.forEach(function (row) {
                _llmRes.forEach(function (lr) { if (lr.q === row.q) { if (lr.eng === "chat_gpt") row.chat_gpt = lr.cited; if (lr.eng === "perplexity") row.perplexity = lr.cited; if (lr.eng === "gemini") row.gemini = lr.cited; } });
              });
              console.log("[AIR] llm top-3 checked (ChatGPT, Perplexity and Google AI)");
            } catch (_le) { if (_le && _le.skip) console.log("[AIR] engine citations came with the dashboard results, the nine checks are skipped"); else console.log("[AIR] llm top-3 enrich error", _le); }
            ...
            _ao.ai_overview.forEach(function (row) {
              ...
              if (!_k) return;
              _pcObj[_k] = { prompt: row.q, page: "", checked: _today, source: "ai_readiness", runs: { chat_gpt: !!row.chat_gpt, perplexity: !!row.perplexity, gemini: !!row.gemini, ai_overview: !!row.cited }, aio: { triggers: !!row.triggers, cited: !!row.cited } };
            });
      ...
      (async () => {
        try {
          ...
          /* v4.16: prompt_answers is written by the dashboard before this run row exists, so the rows carry no run
             until now. The handoff says when the dashboard half started; every answer of this site written since
             then and still without a run gets this one. */
          var _tagSince = null;
          try { const _hoTag = ivaAirHandoff(_snapDom, url); _tagSince = (_hoTag && _hoTag.startedAt) || new Date(Date.now() - 900000).toISOString(); await ivaAirTagAnswers(_snapMid, _snapDom, _snapRunId, _tagSince); } catch (_tg) { console.warn("[AIR] answers not tied to the run", _tg); }
          /* v4.19: the dashboard saves its Google AI answers a few seconds after this run row is
             created, so the pass above runs too early to see them: on the 35-prompt Suave run of
             8 September all 105 assistant answers were tied to the run and all 21 Google AI rows
             were not. The pass is repeated twice more, at four and at twenty seconds, reading the
             same starting time so it can only touch answers of this site written during this run
             that still carry no run of their own. */
          try { var _tagLater = function (ms) { setTimeout(function () { try { ivaAirTagAnswers(_snapMid, _snapDom, _snapRunId, _tagSince); } catch (e3) { console.warn("[AIR] a later tagging pass failed", e3); } }, ms); }; _tagLater(4000); _tagLater(20000); } catch (_tg2) { console.warn("[AIR] the later tagging passes could not be scheduled", _tg2); }
          ...
          recordAirSnapshot({ memberId: _snapMid, domain: _snapDom, url: url, runId: _snapRunId, aiReadiness: aiReadiness, location_code: (window.__ivaAirLoc && window.__ivaAirLoc.location_code) || null, language_code: (window.__ivaAirLoc && window.__ivaAirLoc.language_code) || null, ai_mentions_count: (_mv.ai_mentions_count != null ? _mv.ai_mentions_count : null), ai_search_volume: (_mv.ai_search_volume != null ? _mv.ai_search_volume : null), brand: ((_mv && _mv.brand) || null), ai_overview_count: (_av.ai_overview_count != null ? _av.ai_overview_count : null), prompts_checked: (_av.prompts_checked || null), aioItems: (_av.aioItems || null) });
      ...
      sPLoad(null); sTyp(false);
      /* v4.15: the credit was taken before the work, so a run that ends here leaves the person paying for nothing.
         The charge is returned by its ledger id, which the refund function accepts once and only from its owner. */
      if (_chargeId) { try { await ivaAirRefund(_chargeId, "AI Readiness stopped: " + (e && e.message ? e.message : "unknown error")); } catch (_re) { console.warn("[AIR] refund attempt failed", _re); } }
      bot("Could not analyze this page: " + e.message + (_chargeId ? ". The credit was returned." : "") + " Please check the URL and try again.");
      setStep("url");
    }
  };
