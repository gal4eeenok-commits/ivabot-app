...
function buildResults(d) {
  ...
  if (d.linksStatus === "good") g.push({ title: "Links & Social Profiles", content: (<><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><StatCard number={d.links.internal} label="Internal Links" desc="Connect your pages so Google can discover and rank them." borderColor={NB} /><StatCard number={d.links.external} label="External Links" desc={d.links.external === 0 ? "No external links found. Linking to trusted sources builds credibility." : "Linking to trusted sources builds credibility."} borderColor={d.links.external === 0 ? "rgba(110,43,255,0.2)" : NB} /><StatCard number={d.backlinksCount != null ? d.backlinksCount.toLocaleString() : "N/A"} label="Backlinks" desc={d.backlinksCount === 0 ? "No backlinks detected — this is critical." : d.backlinksCount != null ? `From ${d.referringDomains?.toLocaleString() || "—"} referring domains.` : "Data for this page is not available. Try auditing the homepage for full backlink data."} borderColor={(d.backlinksCount === 0 || d.backlinksCount == null) ? "rgba(110,43,255,0.3)" : NB} /></div>{(d.backlinksCount == null || d.backlinksCount === 0 || (d.backlinksCount != null && d.backlinksCount < 10)) && <div style={{ marginTop: 8, padding: "10px 14px", borderRadius: 8, background: "rgba(110,43,255,0.04)", border: "1px solid rgba(110,43,255,0.15)" }}><div style={{ fontSize: 12, fontWeight: 600, color: C.dark, marginBottom: 2 }}>{d.backlinksCount === 0 ? "No backlinks detected" : d.backlinksCount != null && d.backlinksCount < 10 ? "Low backlink count" : "Backlinks are essential"}</div><div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5 }}>{d.backlinksCount === 0 ? "Without backlinks, it's very hard to rank on page one — even with perfect on-page SEO." : d.backlinksCount != null && d.backlinksCount < 10 ? `Only ${d.backlinksCount} backlink${d.backlinksCount !== 1 ? "s" : ""} found. You're likely losing rankings to competitors with stronger link profiles.` : "Backlinks are one of Google's top 3 ranking factors. Each link from another site is a 'vote of confidence' that helps you rank higher."} See PR & Backlinks section below for opportunities.</div></div>}{d.links.social.length > 0 ? (<div><div style={{ fontSize: 10, fontWeight: 600, color: C.muted, marginBottom: 6 }}>Social Profiles</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{d.links.social.map((s, i) => <SocialBadge key={i} name={typeof s === "string" ? s : s.name} url={typeof s === "string" ? null : s.url} />)}</div></div>) : (<div style={{ fontSize: 12, color: C.muted, padding: "8px 0" }}>No social profiles linked. Consider adding social links to build brand trust.</div>)}<BotNote inline text="A healthy mix of internal links, external references, and social profiles tells Google your site is well-connected and trustworthy." /></>) }); else b.push(d.linksEval);
