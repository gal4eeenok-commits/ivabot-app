/* IvaBot seo-tools v128 - the shell no longer writes its own tracking snapshot. It wrote one without a page address, so the database recorded it against the bare domain and the dashboard drew it as a separate point beside the real one from core-tool, which records the page, the score and the coverage. v127 - the tool select screen carried only the "SEO Tools" title and nothing else, so a signed-in visitor landed on an empty page. It now holds a URL field that starts a Core Audit the same way the home page does, plus links to the other tools and the dashboard. v126 - built from the standalone demo Galyna supplied: only the fill inside the Dashboard link pulses, from clear to a light accent tint and back over 3.4s, with nothing drawn outside the link and no border. The link carries a 1px transparent border and 8px rounding so its box matches the Buy Credits button exactly and the two read as the same size. The demo's prefers-reduced-motion guard is deliberately left out, because Reduce motion on her machine would switch the effect off. Adjust the strength with the single alpha value, 0.10 here, 0.16 in the stronger demo variant. v125 - the link breathed as a soft accent glow around the word, drawn as two blurred outer shadows of 5px and 2px with no spread, so the mist fades out within a few pixels and its footprint stays inside the width of the Buy Credits button beside it. No fill and no border are added, and the 8px rounding matches that button, so the glow keeps its shape while the link's own size and position are untouched. Turn the effect up or down with the two blur values and their alphas. v124 - the link breathed by colour alone, the word moving between the dark text tone and the accent. v123 drew the pulse as an inset ring, which reads as a lilac frame, and its 10px radius against the 8px radius and 1px border of the Buy Credits button beside it made the link look like a smaller button. Nothing about the link's box is touched now, so the row keeps its original geometry. A colour keyframe outranks the inline colour on the link, because animations sit above normal author declarations in the cascade. v123 - the halo stayed inside the Dashboard link. It was drawn as an outward box-shadow spreading 9px, which reached across the 8px gap and pulsed over the Buy Credits button next to it; it is now an inset 1px ring plus the background tint, both of which are painted within the link's own padded box, so nothing can touch a neighbour and the row does not shift. v122 - the breathing starts when the finished audit appears, not on page load: core-tool.js v256 fires iva-audit-ready as it renders the report, the shell holds that in state, and hovering the link ends the effect. Needs core-tool.js v256 for the signal, with window.__ivaLastReportId kept as a fallback. v121 - the breathing is a real CSS animation rendered by React itself: a keyframe in the component stylesheet plus the class iva-dashlink on the link. The v120 requestAnimationFrame loop wrote the glow onto the node it had found at the moment the audit finished, so a React re-render replaced that node and the loop went on painting an element no longer on the page, and the gate window.__ivaLastReportId only ever fires on a fresh paid run. The animation carries no prefers-reduced-motion guard, so Reduce motion on the machine cannot switch it off. It breathes continuously and stops on hover. v120 - the shell no longer falls back to its own legacy Core when window.CoreTool is missing, because that old path skipped charge_credit and saved no checklist, so a failed load of core-tool.js produced a free, incomplete run with no visible error. The shell now renders a reload prompt instead, and the legacy Core JSX stays in the file as unreachable code. v111 — Core routing open to all (whitelist removed). Base v107 — PDF rankings table reverted to the short top-7 list (no Est. Traffic column); the full 200-row list stays on screen + in CSV export + dashboard, not in the PDF. Prior v106: Export CSV button moved to bottom of card. */
(function() {
const { useState, useRef, useEffect, useCallback } = React;
console.log("[IvaBot] seo-tools.js v127 loaded (select screen has a URL field)");
...
const Fold = ({ title, children, open: d = false, borderColor, headerBg, titleColor, count, headerExtra }) => {
...
async function generatePDF(data) {
  ...
  if ((window.IVA_FLAGS && window.IVA_FLAGS.backlinksLive) /* HIDDEN until real DataForSEO endpoints (post 2026-07-01). TODO: same as on-screen — real page-level backlinks (exact count + referring domains) from Backlinks API, target = page URL. Drop "approximate". */ && data.backlinksCount != null) {
    content.push({
      columns: [
...
const ReportV6 = ({ data, onNewAudit, onHome }) => { const { good, bad } = buildResults(data); return (<div style={{ maxWidth: 580, margin: "0 auto", padding: "20px 16px 16px" }}>
  ...
  {data.rankedKeywords?.length > 0 && <div className="reveal reveal-delay-2" style={{ marginBottom: 16, padding: 16, borderRadius: 12, background: C.card, border: `1px solid ${C.cardBorder}` }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
      {(window.IVA_FLAGS && window.IVA_FLAGS.backlinksLive) /* HIDDEN until real DataForSEO endpoints (post 2026-07-01). TODO: replace with REAL organic traffic for THIS audited page (its own page-level traffic), not a domain-level guess. Source: DataForSEO Labs ranked_keywords ETV summed for this page URL, or bulk_traffic_estimation at page level. Then show the real number and rename label to e.g. "Monthly organic traffic" — drop the word "Estimated". */ && data.estTraffic != null && <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 8, background: C.surface, border: `1px solid ${C.cardBorder}` }}><span style={{ fontSize: 16, fontWeight: 700, color: C.dark }}>{fmtVol(data.estTraffic)}</span><span style={{ fontSize: 11, color: C.muted }}>Estimated monthly traffic</span><QM text="An estimate based on your ranking positions and search volumes — not real analytics traffic. Use it to compare potential over time, not as exact monthly visits." /></div>}
    ...
    try {
      ...
      if (!USE_MOCK) {
        ...
        try {
          ...
          console.log("[IvaBot] tracking snapshot left to core-tool, which records the page url, the score and the coverage");
        } catch(e) { console.warn("[IvaBot] tracking note failed:", e); }
      }
      ...
      {view === "select" ? (
        <div style={{ flex: 1, background: "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 24px", gap: 36, maxWidth: 780, width: "100%" }}>
            <div style={{ textAlign: "center" }}><div className="iva-seo-title" style={{ fontWeight: 400, color: C.dark, letterSpacing: "-0.2px", lineHeight: 1, background: "linear-gradient(116deg, rgba(21,20,21,0.25) 8%, #151415 35%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>SEO Tools</div></div>

            {/* v127: the select screen carried only the title, so a signed-in visitor landed on an
                empty page with nothing to do. The URL field starts a Core Audit straight away, the
                same way the home page does, and the tool links cover the other two. */}
            <div style={{ width: "100%", maxWidth: 520, display: "flex", flexDirection: "column", alignItems: "center", gap: 18, marginTop: -12 }}>
              <div style={{ fontSize: 16, color: C.muted, letterSpacing: "-0.2px", textAlign: "center" }}>Paste a URL to run an audit, or pick a tool below.</div>
              <form onSubmit={(e) => { e.preventDefault(); var v = (e.target.elements.url.value || "").trim(); if (!v) return; window.location.href = "/app?tool=core&autorun=1&url=" + encodeURIComponent(v); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "6px 6px 6px 18px", width: "100%", height: 56, background: "rgba(255,255,255,0.45)", border: "1px solid rgba(21,20,21,0.06)", borderRadius: 16, boxSizing: "border-box" }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="#6E2BFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 18, height: 18, flexShrink: 0 }}><path d="M10 13a5 5 0 0 0 7 0l2-2a5 5 0 0 0-7-7l-1 1"></path><path d="M14 11a5 5 0 0 0-7 0l-2 2a5 5 0 0 0 7 7l1-1"></path></svg>
                <input name="url" type="text" inputMode="url" autoComplete="url" spellCheck="false" placeholder="Paste your website URL" aria-label="Your website URL" style={{ flex: "1 1 auto", minWidth: 0, border: "none", outline: "none", background: "transparent", fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 500, color: C.dark, letterSpacing: "-0.1px", padding: "0 0 0 4px" }} />
                <button type="submit" style={{ flexShrink: 0, display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 18px", background: C.dark, color: "#fff", border: "none", borderRadius: 16, fontFamily: "'DM Sans',sans-serif", fontSize: 15, fontWeight: 500, letterSpacing: "-0.3px", cursor: "pointer" }}>Analyze</button>
              </form>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
                <a href="/app?tool=core" style={{ padding: "7px 14px", background: "rgba(110,43,255,0.06)", borderRadius: 999, fontSize: 13.5, fontWeight: 500, color: "rgba(21,20,21,0.66)", textDecoration: "none", letterSpacing: "-0.1px" }}>Core Audit</a>
                <a href="/app?tool=readiness" style={{ padding: "7px 14px", background: "rgba(110,43,255,0.06)", borderRadius: 999, fontSize: 13.5, fontWeight: 500, color: "rgba(21,20,21,0.66)", textDecoration: "none", letterSpacing: "-0.1px" }}>AI Readiness</a>
                <a href="/app?tool=builder" style={{ padding: "7px 14px", background: "rgba(110,43,255,0.06)", borderRadius: 999, fontSize: 13.5, fontWeight: 500, color: "rgba(21,20,21,0.66)", textDecoration: "none", letterSpacing: "-0.1px" }}>Content Builder</a>
                <a href="/dashboard" style={{ padding: "7px 14px", background: "rgba(110,43,255,0.06)", borderRadius: 999, fontSize: 13.5, fontWeight: 500, color: "rgba(21,20,21,0.66)", textDecoration: "none", letterSpacing: "-0.1px" }}>Dashboard</a>
              </div>
            </div>
