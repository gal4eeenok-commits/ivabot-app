/* IvaBot AI Readiness Module v4
   Parses HTML for AI/GEO (Generative Engine Optimization) signals.
   14 checks total: 5 base + 5 advanced + 4 extractability (v4).
...
/* ═══ ADAPTIVE WEIGHTS BY PAGE TYPE ═══
   ...
   Order: schema, faq, llms, bots, qa, og, author, dates, citations, statistics, extractable, tldr, tables, howto
*/
const WEIGHTS_BY_TYPE = {
  homepage: { schema: 15, faq: 12, llms: 15, bots: 11, qa: 5,  og: 10, author: 4,  dates: 0,  citations: 4,  statistics: 7,  extractable: 6, tldr: 6, tables: 3, howto: 2 },
  about:    { schema: 12, faq: 0,  llms: 10, bots: 11, qa: 4,  og: 13, author: 19, dates: 4,  citations: 4,  statistics: 7,  extractable: 7, tldr: 6, tables: 1, howto: 2 },
  article:  { schema: 15, faq: 0,  llms: 4,  bots: 7,  qa: 7,  og: 13, author: 15, dates: 10, citations: 7,  statistics: 7,  extractable: 8, tldr: 5, tables: 1, howto: 1 },
  product:  { schema: 18, faq: 7,  llms: 7,  bots: 8,  qa: 4,  og: 15, author: 4,  dates: 7,  citations: 7,  statistics: 7,  extractable: 6, tldr: 5, tables: 4, howto: 1 },
  pricing:  { schema: 17, faq: 15, llms: 8,  bots: 8,  qa: 7,  og: 10, author: 4,  dates: 0,  citations: 6,  statistics: 8,  extractable: 5, tldr: 5, tables: 6, howto: 1 },
  docs:     { schema: 10, faq: 7,  llms: 10, bots: 8,  qa: 18, og: 7,  author: 7,  dates: 6,  citations: 7,  statistics: 4,  extractable: 6, tldr: 4, tables: 2, howto: 4 },
  contact:  { schema: 19, faq: 0,  llms: 7,  bots: 8,  qa: 4,  og: 13, author: 10, dates: 4,  citations: 0,  statistics: 19, extractable: 5, tldr: 6, tables: 3, howto: 2 },
  other:    { schema: 15, faq: 10, llms: 12, bots: 11, qa: 8,  og: 7,  author: 5,  dates: 4,  citations: 4,  statistics: 8,  extractable: 6, tldr: 5, tables: 3, howto: 2 },
};

/* Verify all rows sum to 100 — assert at load time */
Object.keys(WEIGHTS_BY_TYPE).forEach(type => {
  const w = WEIGHTS_BY_TYPE[type];
  const sum = w.schema + w.faq + w.llms + w.bots + w.qa + w.og + w.author + w.dates + w.citations + w.statistics + w.extractable + w.tldr + w.tables + w.howto;
...
/* ═══ CHECK 11: EXTRACTABLE PASSAGES ═══
   Counts <p> elements with 100-180 words (sweet spot for AI extraction).
   Pages with 3+ such passages score "good".
*/
function checkExtractablePassages(html) {
  if (!html) return { count: 0, total_paragraphs: 0, avg_words: 0, status: "none" };
  ...
  for (const p of paragraphs) {
    ...
    if (words < 5) continue; /* skip empty/tiny paragraphs */
...
/* ═══ CHECK 12: TL;DR / SUMMARY SECTION ═══
   Looks for explicit summary markers OR a substantial first paragraph after H1.
*/
function checkTldrSummary(html) {
  if (!html) return { found: false, type: "none", status: "missing" };

  const cleaned = html
    .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, " ")
    .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, " ");

  /* Limit search to first ~5000 chars of body (where summaries live) */
  const bodyMatch = cleaned.match(/<body[^>]*>([\s\S]*)/i);
  const body = bodyMatch ? bodyMatch[1] : cleaned;
  const firstPart = body.substring(0, 8000);
  const textVersion = firstPart.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").toLowerCase();

  /* Look for explicit markers in first 800 chars of visible text */
  const firstText = textVersion.substring(0, 1500);
  const markers = [
    /\btl[;:]?dr\b/i,
    /\bquick summary\b/i,
    /\bquick answer\b/i,
  ...
  for (const m of markers) {
    if (m.test(firstText)) {
      return { found: true, type: "explicit_marker", status: "good" };
    }
  }

  /* Fallback: look for substantial first <p> after H1 (50-200 words) */
  const h1Match = firstPart.match(/<h1[^>]*>[\s\S]*?<\/h1>/i);
  if (h1Match) {
    const afterH1 = firstPart.substring(firstPart.indexOf(h1Match[0]) + h1Match[0].length);
    const firstP = afterH1.match(/<p[^>]*>([\s\S]*?)<\/p>/i);
    if (firstP) {
      const pText = firstP[1].replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
      const words = pText.split(/\s+/).filter(w => w.length > 0).length;
      if (words >= 50 && words <= 200) {
        return { found: true, type: "intro_paragraph", status: "good", word_count: words };
      }
    }
  }

  return { found: false, type: "none", status: "missing" };
}

/* ═══ CHECK 13: COMPARISON TABLES ═══
   Detects <table> elements with at least 2 columns and 3 rows of data.
*/
function checkComparisonTables(html) {
  if (!html) return { count: 0, tables_found: 0, status: "none" };
  ...
  for (const t of tables) {
    ...
    /* Count columns in first row (use td or th) */
    const firstRowMatch = t.match(/<tr[^>]*>[\s\S]*?<\/tr>/i);
    if (!firstRowMatch) continue;

    const firstRow = firstRowMatch[0];
    const cells = firstRow.match(/<(?:td|th)[^>]*>/gi) || [];
  ...
  return {
    ...
    /* Status */
    status: qualifyingCount >= 1 ? "good" : "none"
  };
}

/* ═══ CHECK 14: HOWTO SCHEMA ═══
   Conditional: only relevant if page has step-by-step content.
   Returns "not_applicable" if no step content (this hides the check in UI).
*/
function checkHowToSchema(html) {
  if (!html) return { applicable: false, has_schema: false, status: "not_applicable" };

  const cleaned = html
    .replace(/<script(?![^>]*application\/ld\+json)[^>]*>[\s\S]*?<\/script>/gi, " ")
    .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, " ");

  /* Detect step content patterns */
  const text = cleaned.replace(/<[^>]+>/g, " ");
  const hasStepText = /\bstep\s*[1-9]\b/i.test(text) || /\bstep\s+(one|two|three|four|five)\b/i.test(text);
  const hasOrderedList = /<ol[^>]*>[\s\S]*?<li[^>]*>[\s\S]*?<li[^>]*>[\s\S]*?<li/i.test(cleaned);

  const hasStepContent = hasStepText || hasOrderedList;

  if (!hasStepContent) {
    return { applicable: false, has_schema: false, status: "not_applicable" };
  }

  /* Detect HowTo schema in JSON-LD blocks */
  const ldBlocks = html.match(/<script[^>]*application\/ld\+json[^>]*>([\s\S]*?)<\/script>/gi) || [];
  let hasHowToSchema = false;

  for (const block of ldBlocks) {
    const inner = block.replace(/<[^>]+>/g, "");
    if (/"@type"\s*:\s*"HowTo"/i.test(inner) || /"@type"\s*:\s*\[\s*[^]]*"HowTo"/i.test(inner)) {
      hasHowToSchema = true;
      break;
    }
  }
  ...
  return {
    applicable: true,
    has_schema: hasHowToSchema,
    has_step_text: hasStepText,
    has_ordered_list: hasOrderedList,
    /* Status */
    status: hasHowToSchema ? "good" : "missing"
  };
}
...
function calculateCheckQuality(checkName, check) {
  ...
  switch (checkName) {
    ...
    case "howto":
      /* If not applicable (no step content on page), give full credit — not a penalty */
      if (check.status === "not_applicable") return 1;
      if (check.status === "good") return 1;
      return 0;
  }
  return 0;
...
function parse(rawHtml, pageUrl, robotsContent, llmsContent, pageType) {
  const checks = {
    ...
    /* Extractability 4 (v4) */
    extractable: checkExtractablePassages(rawHtml),
    tldr: checkTldrSummary(rawHtml),
    tables: checkComparisonTables(rawHtml),
    howto: checkHowToSchema(rawHtml),
  };
  ...
  return {
    ...
    summary: {
      ...
      /* v4 additions */
      extractable_count: checks.extractable.count,
      tldr_found: checks.tldr.found,
      tables_count: checks.tables.count,
      howto_applicable: checks.howto.applicable,
      howto_has_schema: checks.howto.has_schema,
    }
  };
}
