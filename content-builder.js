...
console.log("[IvaBot] content-builder.js v50 loaded");
...
/* v50: Frontend keyword highlighting — full phrases + individual words from phrases */
...
let out=rawHtml;
/* Pass 1: highlight full keyword phrases */
kwList.forEach(k=>{out=hlOne(out,k.keyword,k.freq||"MV");});
/* Pass 2: highlight individual words from phrases (>3 chars) that aren't already inside <mark> */
const wordsSeen=new Set();
kwList.forEach(k=>{(k.keyword||"").split(/\s+/).filter(w=>w.length>3).forEach(w=>{const wl=w.toLowerCase();if(!wordsSeen.has(wl)){wordsSeen.add(wl);out=hlOne(out,w,"LV");}});});
return out;};
