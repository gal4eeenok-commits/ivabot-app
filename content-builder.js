/* IvaBot Content Builder v72 â brand guide upload (read file, 15K limit, send to GPT) */
(function() {
const{useState,useRef,useEffect,useCallback}=React;
console.log("[IvaBot] content-builder.js v72 loaded");

/* âââ CONFIG â single Edge Function endpoint âââ */
...
/* âââ API HELPERS â v45: direct Edge Function, no Make âââ */
...
async function callChat(context, chatHistory, question) {
  ...
  try {
    ...
    if (typeof answer === "string") {
      ...
      answer = answer.replace(/\\n/g, '\n').replace(/\\"/g, '"').replace(/\*\*/g, '').replace(/###\s?/g, '').replace(/^- /gm, 'â¢ ');
...
/* Get user ID from any available source â Supabase Auth */
...
const PAGE_TYPE_CONFIG = {
  ...
  "product page": {
    extraQ: "Tell me about the product â what is it, what's special about it?",
...
function getPageConfig(pt) {
  ...
  let normalized = t
    ...
    .replace(/\b(main page|front page|home page|home|main|front|Ð³Ð»Ð°Ð²Ð½Ð°Ñ|Ð´Ð¾Ð¼Ð°ÑÐ½ÑÑ)\b/g,"homepage")
...
const LOCATION_CODES = {
  ...
  "spain": 2724, "es": 2724,
  "italy": 2380, "it": 2380,
  "ukraine": 2804, "ua": 2804, "ÑÐºÑÐ°ÑÐ½Ð°": 2804, "ÑÐºÑÐ°Ð¸Ð½Ð°": 2804,
...
const fmtVol = v => { if (!v) return "â"; if (v >= 1000) return (v / 1000).toFixed(1).replace(/\.0$/, "") + "K"; return String(v); };
...
/* âââ FIX #2: Loading bar that waits for API âââ */
...
const readFileAsText=async(file)=>{
  ...
  /* .doc/.docx â read as text (basic, won't get formatting) */
...
/* âââ EXTRAS BLOCK â shows related/paa/autocomplete inline with keywords âââ */
const ExtrasBlock=({extra})=>{
  ...
  return<div style={{marginTop:8}}>
    ...
    {o&&<div style={{marginTop:6,padding:12,borderRadius:10,background:"rgba(110,43,255,0.02)",border:`1px solid ${C.cardBorder}`,fontSize:11}}>
      ...
      {extra.paa?.length>0&&<><div style={{fontSize:9,fontWeight:600,color:C.accent,marginBottom:3,textTransform:"uppercase",letterSpacing:"0.5px"}}>PEOPLE ALSO ASK</div><div style={{lineHeight:1.7,marginBottom:10,fontSize:11}}>{extra.paa.map((q,i)=><div key={i}>â¢ {typeof q==="string"?q:q.question||q}</div>)}</div></>}
...
/* HL prep â shared between hlF (React) and hlHtml (string) */
...
/* hlF â highlight keywords in plain text, returns React elements */
...
/* v67b: Exact + Partial keyword highlighting â NO nested marks. */
...
/* âââ LOADING STEP LABELS âââ */
...
/* âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
   MAIN COMPONENT â v48: Fixed keyword selection tracking + title generation
...
/* âââ PHASE 2: AI Chat (questions only, after structure) âââ */
...
/* âââ DFS ENRICHMENT (shared helper) âââ */
...
/* âââ SHOW KEYWORDS (shared UI builder) âââ */
const showKeywords=(enriched,extras,adjustsLeft)=>{
  ...
  add("b",<div>
    ...
    <BotTip short="Each keyword has search volume, competition, and priority."><div><div style={{marginBottom:6}}>Vol. â how many people search this per month.</div><div style={{marginBottom:6}}>KD â competition (0â100). Lower = easier to rank.</div><div>HV = High Volume, main keyword. MV = Medium. LV = Low.</div></div></BotTip>
    <KwS keywords={enriched} init={init} onDone={s=>{sSkw(s);skwRef.current=s;sConfirmedKeywords(enriched.filter(k=>s.includes(k.keyword)));kwDone();}} onAdj={(currentSelection)=>{
      ...
      setStep("ka");bot("Just tell me what to change â add, remove, or refine keywords.");
    }}/>
    <ExtrasBlock extra={extras}/>
  </div>);
};

/* âââ KEYWORD DONE â Phase 1 continues âââ */
const kwDone=()=>{
  mk("kw");
  setStep("gl");
  bot(<div><div style={{marginBottom:8}}>Great keywords! Now let's shape your content.</div><div style={{fontWeight:600,marginBottom:6}}>Step 1 of 4 â What should this page achieve?</div><div style={{color:C.muted,fontSize:12,marginBottom:6}}>This shapes the content strategy and call-to-action.</div><ExBox items={["Sell a product","Get leads through a contact form","Educate visitors and build trust","Explain a service","Get sign-ups or bookings"]}/></div>);
};

/* âââ KEYWORD ADJUST (up to 5 rounds) âââ */
...
/* âââ GENERATE STRUCTURE âââ */
const gStr=async()=>{
...
const gCnt=async()=>{
  ...
  try {
    ...
    /* v49: Send only primary+secondary as exact keywords (max 3, like Typebot). Supporting â individual words. Market â supporting words. */
    ...
    const kwPlacement=[];
    /* Keyword density based on Typebot formula (charsâwords): <140w=1x, 140-350w=2x, 350-700w=3x, 700-1000w=4x, 1200+=5x */
    ...
    if(classifiedContent.primary){
      ...
      kwPlacement.push({keyword:pk,role:"primary",placement:`Insert this exact phrase ${priCount} times total: 1 time in the intro paragraph before H1, and ${priCount-1} more time(s) spread evenly across body paragraphs. Do NOT cluster them â space them out.`});
    }
    ...
    kwPlacement.forEach((kw,i)=>{
      ...
      else{console.log(`  [${kw.role}] "${kw.keyword}" â ${kw.placement}`);}
    });
    console.log("[CB] ==============================");
    ...
    /* v50: H3 post-processing â convert bold paragraphs back to <h3> for known H3 headings from structure */
    ...
    /* v49: Keyword spam postprocessor â count exact phrases, auto-fix if overspammed */
    if(html.length>50){
      try{
        ...
        if(spamIssues.length>0||wordSpam.length>0){
          ...
          wordSpam.slice(0,3).forEach(([w,c])=>{
            fixInstructions.push(`The word "${w}" appears ${c} times â reduce to 4-5 by using synonyms or removing.`);
          });
    ...
    if(isMobile)sMTab("panel");
    bot(<div><div style={{marginBottom:6}}>Your full content is ready!</div><div style={{color:C.muted,fontSize:12,marginBottom:6}}>I slightly adjusted some headings to distribute your keywords evenly across the page â this helps with SEO.</div><div style={{color:C.muted,fontSize:12}}>Want changes? Just describe what to fix.</div></div>);
  } catch(err){console.error("[CB] gCnt error:",err);stopLoading();sTyp(false);sPLoad(null);bot("Something went wrong. Please try again.");}
};

/* âââ TWEAK (structure or content) âââ */
...
/* âââ PDF EXPORT (pdfmake) âââ */
async function generateCBPDF(briefData, contentHtml, panelMode) {
  ...
  /* ââ SEO TITLE & META DESCRIPTION ââ */
  ...
  /* ââ ADDITIONAL SEARCH PHRASES ââ */
  ...
  /* ââ FULL CONTENT (if panelMode === "ct") ââ */
    ...
    const parseHtmlToPdf = (html) => {
      ...
      parts.forEach(part => {
        ...
        bodyParts.forEach(bp => {
          ...
          if (plainText) {
            /* Check for bold fragments â extract <strong> sections */
  ...
  pdfDocGen.getBlob(async (pdfBlob) => {
    try {
      ...
      if (upMemberId && pdfBlob) {
        ...
        console.log("[CB] PDF not uploaded â no member ID");
...
/* âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
   SEND â v48: Phase 1 = strict (no chat). Phase 2 = tweak + questions.
   âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ */
const send=()=>{
  ...
  /* === PHASE 1: strict linear â input = answer === */
  ...
  if(step==="ec"){
    ...
    if(/\b(my|own|have|paste|use)\b/i.test(tl)||t.includes(",")){mk("e");setStep("ok");bot(<div><div style={{fontWeight:600,marginBottom:6}}>Type your keyword phrases below, separated by commas.</div><div style={{color:C.muted,fontSize:12,marginBottom:6}}>Use 2â3 word phrases. Up to 7 phrases max.</div><ExBox items={["brazilian coffee beans, vegan coffee, organic roast","handmade silver rings, custom jewelry, boho rings","yoga for beginners, morning stretches, home yoga"]}/></div>);return;}
  ...
  /* === PHASE 2: chat open â tweak or question === */
...
/* âââ START KEYWORD GENERATION âââ */
