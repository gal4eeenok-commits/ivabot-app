...
/* v66: + partial match (2 of 3 words) highlighted with lighter color + headings highlighted */
...
const buildPartialRegexes=(kw)=>{const words=kw.split(/\s+/).filter(w=>w.length>0);if(words.length<2)return[];const escaped=words.map(w=>w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'));const pairs=[];for(let i=0;i<escaped.length;i++){for(let j=i+1;j<escaped.length;j++){pairs.push(new RegExp(`\\b(${escaped[i]}(?:\\s+${STOP_WORDS}){0,3}\\s+${escaped[j]})\\b`,'gi'));}}return pairs;};
