    ...
    try {
      ...
      const _pAio = (async () => {
        try {
          ...
          var _rk = await ivaFetchRetry(DFS_PROXY, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + (await ivaAuthToken()) }, body: JSON.stringify({ mode: "ranked_only", domain: _h, location_code: _airLoc.location_code, language_code: _airLoc.language_code }) }).then(function (r) { return (r && r.ok) ? r.json() : null; });
          var _pk = _nrm(url);
          ...
          if(_tracked.length){ console.log("[AIR] using the prompts you track:", _tracked.length, "so no questions are invented"); }
